...
`AT-194` keys on the **marginal delta** attributable to the addendum. Carried in
`.dev-flow/BACKLOG-CODE.md`, not closed here.
...
- Status: Added in batch `2026-07-27-batch-64` (US-B64-1/2, HLR-103, LLR-103.1…103.6;
  ...
  `tests/goldens/` unchanged — no golden was re-baselined. **Residuals carried, not closed** — the
  named carries are (a) the two uncapped neighbouring tables at 988 B/entry, (c) the uncapped region
  cardinality `R`, (e) the relocated `O(R)` attribution walk (`500 → 128000`), which capping `R`
  would bound together with (c), the addendum's notice not reaching the `## Truncation appendix`
  (three `> TRUNCATED:` emitters, two feed the appendix and two do not), the un-named severity of a
  dropped ERROR, `ADDENDUM_NOTICE_VARIANTS_MAX` being possibly low for `V > 8` projects, and
  `changes/apply.py::_first_intersecting_symbol`, which carries its own local copy of the
  overlapping-range defect and so would **not** be repaired by unfreezing `range_index.py`.
