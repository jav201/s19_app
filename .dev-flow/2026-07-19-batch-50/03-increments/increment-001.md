...
## 8. CI-fragility catch (fail-loud addendum, 2026-07-20)
PR #99's first `tui-ci` run (Python 3.11, runtime-only, **no ruff installed**) FAILED on `TC-094`: `python -m ruff` exits 1 with `No module named ruff` — which is NOT a `FileNotFoundError` (python exists), so the original skip guard missed it and the `returncode==0` assertion failed. AT-094 (behavior parity, no ruff dependency) passed — the failure was isolated to the lint-check node. Local + full-guard-host runs never caught it (ruff present locally). **Same class as batch-48 `test_tc081_4` CI-fragility.** Fix (`d40b60f`, test-only): guard TC-094 with `importlib.util.find_spec("ruff")` → `pytest.skip` when ruff isn't importable; runs where ruff is installed (dev/pre-commit), skips honestly on the runtime-only CI job. Re-verified locally (2 passed, ruff importable=True). CI re-run in progress. **Lesson:** a pytest node that shells to a dev-only tool must gate on the tool's importability, not just `FileNotFoundError` — `python -m <missing-module>` returns exit 1, not a missing-executable error.
