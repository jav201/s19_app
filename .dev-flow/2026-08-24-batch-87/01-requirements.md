...
| **P-13** | `AT-B87-*` / `TC-B87-*` are letter-initial and therefore ungoverned by the id registry, so no reservation is owed | PREMISE | ✅ TRUE | M-1 tail: `grep -c "AT-B86" AT-TC-REGISTRY.jsonl` → **0** and `grep -c "AT-B85"` → **0`; `_meta.next_free.AT` reads **282** and is untouched by either predecessor batch | precedent is consistent across batch-85, batch-86 and this batch; the registration question is recorded as D-87-F rather than silently decided |
...
| **P-15** | The two in-source claims about the shell's arity are still true | PREMISE | ❌ **FALSE — two stale claims, measured** | M-6: `app.py:1849` says *"an 8-child `#workspace_body`"* and `app.py:1880` says *"the other seven screen containers"*, while the compose body yields **ten** (`app.py:1907-1916`) and `action_show_screen`'s docstring (`app.py:1885`) says *"hiding the other eight"*. `styles.tcss:46` repeats *"the 8-screen `#workspace_body`"* | the HLR-85.2 defect class, alive on this surface. **NOT fixed here — this is a spec-only batch and they live in source.** Registered as R-87-3 with the exact sites, so the next batch that touches `app.py` inherits the enumeration rather than the category |
...
### M-4 — `_ifc_corpus` under a duplicate COMPONENT id, five arms, all executed

Synthetic corpora only. Arms 1 and 4 answer D-87-A; arms 2 and 3 answer D-87-D; arm 5 answers D-87-B.
...
ARM 5 - the parent declares ZERO outputs (real corpus copied to temp, plus a synthetic workspace_body)
...
**Arm 5 is the one that settles D-87-B**, and it is a C-55 instance: the emits BLOCK of arm 1 —
same corpus, same child — simply disappeared. A parent with no outputs silently switches off the
emits half of balancing for every child it has.

Arm 5 also enumerated, for free, exactly what a declared `workspace_body` owes:
...
That list is thirty ids, printed by the rule rather than assembled by hand. It is the re-export set
of §5.3, updated for the split (`slot_rows` becomes two).
...
### AT-B87-04 — the batch's source diff is empty and the suite ledger holds

- **Observable outcome:**
  `git diff --stat $(git merge-base HEAD origin/main) -- s19_app/ tests/ tools/ pyproject.toml`
  reports 0 files / 0 insertions / 0 deletions at the gate, and the whole-suite run matches the
  recorded base ledger.
- **Shipped surface:** git + the orchestrator's gate suite run (the orchestrator owns suite runs;
  this document does not execute pytest).
...
- **Boundary catalog:** ☑ empty — trivially observable · ☑ boundary — the diff scope names the four
  source paths explicitly · ☐ invalid / ☐ error — N/A: a one-command structural check.
...
| US-87-1 | `loaded_panel` balances at six outputs; one declaration, not two | `devflow-validate.py` | `AT-B87-02` | RED arm ✅ live pre-fix run |
| US-87-2 | The single V12 finding names `workspace_body`/`workspace_shell` | `devflow-validate.py` | `AT-B87-03` | pre-state ✅ M-1 |
| US-87-1 + US-87-2 | 0 product files touched, suite-neutral | git + gate suite | `AT-B87-04` | measured at gate |

### Batch acceptance criteria

- **3** HLRs and **8** LLRs covered by **8** inspection/analysis TCs (`TC-B87-01` … `TC-B87-08`); the
  two stories by **4** ATs.
- **0 BLOCK attributable to this batch** in the post-state run — stated WITH its scope: the batch's
  own edits produced 12 BLOCKs mid-authoring (8 V21 owner BLOCKs before the `LLR-87.*` headings
  existed, 4 V20 Atlas-staleness BLOCKs), all discharged inside the batch and recorded in §5.6. A
  criterion without that scope would be the unfalsifiable batch-85 §6.3 shape.
...
| **G8** | per-id canon greps ≥ 1 for every batch-87 heading id — **executed AT THE P3 INCREMENT GATE**, one grep per id, one id per line (the batch-86 M7 lesson), with CLOSE as the confirming re-measurement | OPEN — owed by the Phase-3 seeding increment |
| **G9** (PIN) | source-scope diff empty + ledger `post = base - 0 + 0` — labelled a pin, not a gate | GREEN (pin) |

**Kill-mutation disposition.** M2/M3/M6/M8 of the standing protocol are discharged by CITATION of
the shipped `--selftest` arms (192 arms, exit 0). **This batch additionally carries three LIVE RED
arms taken on the REAL corpus rather than in memory**, which is stronger evidence than batch-86 had:
(i) V21 BLOCKed eight times, naming each `LLR-87.*` owner, while the headings did not yet exist;
(ii) V12 BLOCKed on `emits artifact_row_set, unload_all_row` while the batch-86 re-export still said
`slot_rows`; (iii) V20 BLOCKed on all four derived files after the first contract edit. Each went
green by the intended change and by nothing else. **Still OWED batch-local at Phase 4, on a copy:**
the consumers-entry mutation (remove one entry → the V13 PAIR count moves +1 while the file-set
union may not) and the canon-mirror grep mutation. Never the live tree (C-40).
...
- **Priority:** high
- **Acceptance (black-box):** `AT-B87-03` (§3).
...
- **Priority:** medium
- **Acceptance (black-box):** `AT-B87-04` covers the neutrality half; the figure half is Layer-A analysis by design (partially unobservable, declared — pilot HLR-85.4 precedent: a green check never means "the cost is correct").
...
- **Numeric pass threshold:** **31** re-exported ids, equal to the count of outputs `screen_workspace` declares; **0** V12 emits BLOCKs and **0** V12 consumes BLOCKs; exactly **1** V12 NOTICE, naming `workspace_body` and `workspace_shell`; `PARENT : SYSTEM` appears **0** times in this document.
...
| **D-87-B** | `workspace_body` declares its **full** output set — eleven own plus thirty-one re-exports | declare it with no OUTPUTS, which parses and passes | M-4 arm 5: with the parent's OUTPUTS empty, `screen_workspace`'s emits BLOCK **disappeared** — `devflow-validate.py:484` skips the emits half when either side is empty. The cheap form does not weaken balancing; it switches it off for every child, which is the exact "check that cannot fail" this programme exists to kill | 42 outputs, ~120 consumer entries, and the duplication measured in §5.6 |
...
| 4 | a TC invariant RED on arrival | **Not reproduced** — LLR-87.4's invariant ("no indented line follows the block") was measured on the file before it was written as a threshold |
...
> **Every line of the transcript below is written flush left.** Inside this document that is a
...
```
after the batch-85 contract was re-authored and before the batch-86 re-export followed it:
...
... seven further V21 BLOCKs, one per output owner, across both edited records ...
[x] V20  four BLOCKs, one per derived Atlas file
12 block - 254 notice - 13 not applicable
```

Each of the three arms went green by the intended change and by nothing else: the V12 arm by
LLR-87.5's re-export update, the eight V21 arms by this document declaring the `LLR-87.*` headings,
the four V20 arms by the Atlas regeneration.
...
| **Σ** | 51 | **102** — plus 2 no-literal notices (`screen_workspace/empty_state` and `workspace_body/empty_state`, the same type-addressed widget reported once per declaration) |
...
|---|---|---|---|---|---|
| 1 | Declared OUTPUTS | **42** (11 own + 31 re-export) | 30 | 6 (re-authored; 5 as the pilot shipped) | M-6/M-7 + the post-state V11 |
| 2 | Consumer entries · distinct dependant files | **139** entries (45 of them on own outputs) · **20** files (11 on own outputs) | **94** · 15 | **18** · 6 | M-7/M-8/M-9 + a direct parse of each block, cross-checked against the post-state V14 total of 251 = 18 + 94 + 139 |
| 3 | Addresses literal-greppable | **41 of 42** (`empty_state` type-only) | 29 of 30 | 6 of 6 | M-7 |
| 4 | V13 stray pairs, classified | **62 pairs · 0 undeclared dependants** (26 own + 36 duplicated) | 36 · 0 | 4 · 0 | post-state census above |
| 5 | Stale in-repo claims found | **4 sites in 2 files** — SURVEYED, not declared-absent: `app.py:1849`, `app.py:1880`, `app.py:1885`, `styles.tcss:46` (R-87-3) | not surveyed — declared absence | 5 sites / 4 files | M-6 |
| 6 | Authoring effort | captured at close | captured at close | captured at close | disposition, not M-cited |

**Dispersion (n = 3, range form — no mean, no total):** outputs **6 → 30 → 42**; consumer entries
**18 → 94 → 139**; stray pairs **4 → 36 → 62**. The spread is a factor of seven on outputs and
...
| **R-87-3** | **Four in-source claims about the shell's arity are false**, and they are the HLR-85.2 defect class alive on this surface: `app.py:1849` ("an 8-child `#workspace_body`"), `app.py:1880` ("the other seven screen containers"), `app.py:1885` ("hiding the other eight"), `styles.tcss:46` ("the 8-screen `#workspace_body`"). Measured: ten children (M-6) | New, enumerated exhaustively rather than described by a category. **Not fixed here** — spec-only batch. Owed by the next batch that edits those files |
