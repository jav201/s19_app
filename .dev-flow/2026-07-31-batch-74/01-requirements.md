...
| **LLR-106.1** | The `Address` cell **shall** render at most `REPORT_ADDRESS_CHARS` characters. The bound **shall** be applied without materialising the full hex rendering — the full string *is* the allocation being bounded, so the implementation **shall** derive the leading digits arithmetically (e.g. by right-shifting) rather than formatting-then-slicing. | `_modifications_lines`, `_checklist_lines` |
| **LLR-106.2** | A truncated `Address` **shall** render in a form that **fails** `^-?0x[0-9A-F]+$` and states how many hex digits were elided. An untruncated `Address` **shall** be byte-identical to the shipped `f"0x{n:08X}"` rendering. | both producers |
...
| **AT-226** | **2** / 106.1+106.2 | A wire-legal `'0x'+'F'*100000` address renders `≤ REPORT_ADDRESS_CHARS`, **and** the emitted token **fails** `^-?0x[0-9A-F]+$`, **and** states the elided digit count | truncate with plain `[:n]` → the token still matches `^0x[0-9A-F]+$` while understating by `2^…`. **This is the forgery, and it is the point of the story** |
...
sentinel"**. **Every AT and TC has exactly one owning increment.**
