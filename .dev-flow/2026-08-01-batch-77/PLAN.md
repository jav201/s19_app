...
| **D-16** | **Autonomy extended THROUGH TO MERGE** — file-cap extensions for indivisible work no longer escalate | **OPERATOR** 2026-08-03, *"continua autónomo hasta merge"* | Supersedes the per-increment escalation of R-12/R-13. Every extension is still **recorded and named**, never silent. Inc-5 exercised it immediately: 11 files, not 5. |
| **D-17** | **`C-77-m` resolved by OPTION (b) — re-point the six `R-TUI-112` citations at `R-TUI-072`** | **autonomous** | §6.5 **Amendment A amends `R-TUI-072` IN PLACE**; it does not supersede it. So the governing id *is* `R-TUI-072` as amended, and `R-TUI-112` was minted by Inc-4 in comments as a drafting accident, not a decision. Option (a) — creating an `R-TUI-112` row — would **contradict the amendment actually made**, duplicate an amended requirement, and open a new requirement surface (row, traceability, validation) for zero gain. Folded into **Inc-6**, which opens `screens_directionb.py` anyway. |
| **F-7** 🆕 | 🛑 **THIS BATCH COMMITTED `C-77-i`'S EXACT DEFECT, TWO INCREMENTS AFTER CATALOGUING IT** | found at Inc-5 | `R-TUI-112` is cited at **6 sites** in shipped source and tests (`screens_directionb.py:612`, `:1454`; `test_legend_two_pane.py:705`; `test_tui_snapshot.py:671`, `:919`, `:957`) with **no row in `REQUIREMENTS.md`** (high-water `R-TUI-102`). `C-77-i` exists precisely to correct a *false claim about a dangling requirement id* — and Inc-4 minted a genuinely dangling one while the batch was fixing it. **Cataloguing a defect class does not inoculate against it.** |
| **F-8** 🆕 | **The census integer was low for the FOURTH consecutive revision — and the three new miss modes are each different** | measured at Inc-5 | Executed: **35 statement sites across 13 files** (Inc-4: 7/4 · Inc-5: 28/9) against the document's *"9 known, ~14 more"* ≈ 23 — **52 % higher**. Site 7 proved an **id**-keyed grep cannot find prose. These prove more: `REQUIREMENTS.md:4798` — **a requirement restates its contract in its own `Validation:` row**, so a census reading only Statements misses it every time · `01b:65`, `increment-05.md:131` — written **`5-tick`, hyphenated**, invisible to every prior sweep searching `5 tick` · `traceability-matrix.md:92` — **a second row in a file already named at `:54`**, invisible to a line-keyed census. **Only a claim-derived superset sweep (`ruler|tick`, 327 hits, every one read) closed it.** The agent then miscounted its own summary (*"33 sites in 11 files"* — 11 was files *edited*) and self-caught it by re-adding the per-file column instead of trusting the summary. |
| **D-18** | **batch-47's PROCESS artifacts are MARKED, not rewritten** | **autonomous**, agent-proposed | Normative sites (`REQUIREMENTS.md`, `LLR-072.3`) and living docs (`06-docs/`) are **corrected in place**. But `01b`, `02-review`, `increment-05/06` and `04-validation` are a **historical record**: batch-47 genuinely shipped a 5-tick ruler and those verdicts genuinely held at the time. Each surviving statement carries an inline `⚠️ RETIRED at batch-77` marker. **Rewriting a validation record to describe behaviour it never validated would falsify it** — the census exists to stop false claims, not to create one. |
