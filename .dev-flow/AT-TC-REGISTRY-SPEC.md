...
|---|---|---|---|
| Batch-scoped | `AT-B64-04`, `AT-N8-07` | **Outside** the allocation authority and outside the guard | Cannot collide across batches by construction — the batch tag is the namespace. **New work should prefer these**; every batch-scoped id is one the global pool never has to carry. ⚠️ **Outside the guard is also outside the GRAMMAR, which this cell alone does not say** — see §10: no guard reports a batch-scoped id at all, including one nobody registered |
...
| Batch-scoped ids (§2.3) escape the guard entirely | ⚠️ **Split verdict — amended in place 2026-08-21.** The **collision** risk is ✅ accepted, deliberately: they cannot collide across batches, and widening the guard for a class with **zero** recorded collisions doubles its complexity. The **grammar** consequence is **NOT** accepted and stays an open **P1**: `_FUNC_ID_RE` (`tools/id_registry.py:95`) requires digits immediately after `at`/`tc`, so `at_b78_09` never parses, the token is classified `governed=False`, and **G1, G3 and G5 all decline to police it — silently**. **Re-derived 2026-08-21, not copied:** `grep -rhE "^def test_(at|tc)_b[0-9]+_" tests/*.py | wc -l` = **141** batch-scoped test nodes, against **0** registry rows (`grep -cE '"(AT|TC)-B[0-9]+' AT-TC-REGISTRY.jsonl`). The narrower batch-79 pattern that first surfaced this read **108** at the Inc-11 close and reads **109** today — **the count drifts, the `0` does not**, which is why the `0` is the figure that matters. `tests/*.py` is deliberate: a recursive grep also matches tracked `.pyc` binaries and inflates it. **Collision-freedom does not discharge grammar-blindness**; the row this replaces conflated the two and that conflation is what let the project assert both stances at once. Tracked in `BACKLOG-CODE.md` § *"The AT/TC registry is BLIND to all 106 batch-78/79 acceptance nodes"*. Widening the regex reclassifies tokens repo-wide and could redden G1/G3/G5 across 678 existing ids — **its own batch**, not a row edit |
