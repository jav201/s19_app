...
| **A-43** | D-10 / A-23's sufficiency claim (**refuted at Inc-1 by its own battery**) | "**Every** file-derived interpolation in `report_service.py` carries a literal prefix … so a field cannot reach column 0 and no single-line block starter can fire." Both Phase-2 reviewers converged on this and I adopted it. | **FALSE when the literal prefix is itself a list marker** — and `f"- {md_safe(...)}"` is a real emission shape (`flow_report_service.py:400`). Measured behind a `- ` prefix: `1) item` → `ordered_list_open`; `---` → `hr`; **`- item` → a nested list whose marker is CONSUMED, so the reader sees `item` while the file says `- item`** — it opens no construct, so a grammar-only assertion scores it inert while the display silently loses two characters. Behind `\| ` all three are inert, which is why the claim looked true. **NEW:** Mode A escapes `-`, `+`, `=` **in leading position only**, and a leading `\d{1,9})` gets its delimiter escaped (`1.` was already dead — `.` is escaped unconditionally). Interior `-` is untouched, so `mac-linked` keeps rendering as `mac-linked` and the readability objection that killed whole-set escaping does not apply. The newline collapse remains pin 1; this is a **second** control, not a replacement. | Inc-1 battery |
...
| **Inc-1 finding — D-10's prefix argument is insufficient** | A-43 |
