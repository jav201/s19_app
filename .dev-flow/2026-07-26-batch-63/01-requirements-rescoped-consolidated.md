...
| `AT-173` | The same for `write_flow_report`, **patching `flow_report_service.document_bytes`** — the binding in the module under test. | Enforces the D3-C scope ruling; that module's `fits()` (`:310`) gates **emission**. **The patch target is normative:** `flow_report_service.py:69-74` uses `from .report_service import (...)`, so patching `report_service.document_bytes` leaves the already-bound name untouched and the AT would pass against an unfixed flow writer. |
