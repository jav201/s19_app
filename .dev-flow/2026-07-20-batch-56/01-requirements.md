...
- **Statement:** The summer **shall** use `DATATYPE_SIZES.get(...)` (never subscript — subscript raises `KeyError`), **shall** length-guard token access (`len(tokens) >= 2` before `token[1]` in the alignment collector; `len(tokens) >= 3` before `token[2]` in the walk), **shall** parse each alignment value AND each `max_axis_points` inside a `try/except (ValueError, TypeError)` returning `None` (NEVER an `isdigit()`/regex pre-predicate — sec-F3 inherited), **shall** implement `align_up(o, a)` so that it short-circuits (`return o`) when `a <= 1` BEFORE computing `o % a` (so a non-positive alignment can never raise `ZeroDivisionError` — Phase-2 sec-M3; defense-in-depth with the LLR-A56.2 non-positive guard), and **shall** return `None` (never raise) for a garbage datatype, a non-int alignment value, a **non-positive alignment value**, a truncated line, an absent axis count, or an over-cap offset, so that a malformed alignment-declaring CURVE/MAP parses to grey without aborting the load.
...
**SPLICED + RECONCILED at Phase 2 → see [`02-review.md` §2](02-review.md) (authoritative).** The Phase-2 fold reconciled the two Phase-1 docs into one canonical registry:
- **Gate-blocking AT (main PR):** AT-113 (multi-class 16/packed 13 + output-then-consume), AT-114 (demo 25/51/12/None + 0 drift, **anchor**, RED=MOD_COMMON-honored 26), AT-115 (R-A isolation 16>13 ∧ without==13), AT-116 (unmodeled directive→None), AT-117 (pad=0 boundary→10), AT-118 (over-align→16≠12), AT-119 (**R-C no-trailing→17**, RED 24), AT-120 (DoS→None fast), **AT-122 (hostile alignment value non-int/0/negative→None, no exception — Phase-2 sec-M3/qa-M4)**. **Post-merge:** AT-121 (re-freeze).
- **TC (LLR→TC):** TC-143 census (C-31: key-set AND value-set derived) · TC-144 walk (16 + secondary 8) · TC-145 collector (incl. `x`/`0`/`-4`→None) · TC-146 `align_up` (incl. `a<=1` guard, no raise) · TC-147 packed · TC-148 MOD_COMMON-excl/full-span-or-None · TC-149 DoS · TC-150 R-C=17 · TC-151 supersede TC-133b · TC-152 unfreeze · TC-153 re-freeze.
- **US/HLR/LLR ids** stay this doc's `*-A56` scheme; the catalog's `LLR-ALIGN.x` re-peg per `02-review.md` §2. **New positive/negative TCs land in a NEW non-frozen `tests/test_a2l_alignment_sizing.py`** (imports `_write_a2l`/`_axis_meta` from `tests/test_a2l_inline_axis_length.py`); the TC-133b supersede edits `tests/test_a2l_inline_axis_length.py` in place. C-12 chain head = `parse_a2l_file`.
