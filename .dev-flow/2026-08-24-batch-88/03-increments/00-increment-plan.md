...
| **Withheld until** | ~~the `security-reviewer` lens~~ — **RELEASED 2026-08-26**, `02-review-security.md`. Two rulings travel with it: the entry is **floored at `>= 10`**, and ~~F2's silent-failure risk is **accepted with its mitigation declined** to hold the edit at 4 lines~~ — ⚠ **SUPERSEDED the same day: the ruling was REVISED 2026-08-26 to ADOPT the guard and widen to 5 edits.** Corrected 2026-08-27; the struck text is left visible because this file is the baseline, but it stated a withdrawn ruling as current |
...
| Gate | `AT-B88-09`: installing `.[evidence]` yields an importable imaging library **and** a `textual` that is `>= 8.0.2` and **not** the `8.2.8` pin; `.[dev]` yields the pin; **bare `-e .` must FAIL** the import. `.github/` shows exactly **1** file and **5 EDITS** ~~lines~~ (the fifth is the import guard). ⚠ **EDITS, not diff lines** (2026-08-27, `R-88-8`'s fourth instance): a rerouted line is a delete plus an add, so the git shape corresponding to these five edits is `1 file changed, 2 insertions(+), 4 deletions(-)`, one hunk, `tui-ci` job only — **that pair is the check** |
