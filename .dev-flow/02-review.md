...
| Q-N01 | major | Javier (jav201) | This batch — fold into Phase 3 increment 1 review packet | TC-047 (junction probe, the only test closing blocker S-002) is `pytest.mark.skipif`-gated to Windows. CI is `ubuntu-latest` only, so the test never runs in CI. Risk: blocker S-002 silently uncovered. Mitigate by attaching `pytest -q tests/test_tui_workspace.py::TestCopyIntoWorkareaContainment::test_junction_rejection` stdout from a manual Windows run to the increment 1 packet before the Phase 4 gate. |
