...
- **`git diff main` tests aren't CI-portable (escaped to the PR gate).** `TC-frozen-diff` diffed against `main`, which exists locally but NOT in CI (actions/checkout fetches only the PR head) → `fatal: bad revision 'main'`, exit 128, the only red in an otherwise-green 906. The canonical directionb guard masks this by reading only stdout (trivially empty when the ref is absent — a silent false-green). Fix: resolve a real base ref (`origin/main` → `main`) and `pytest.skip` if neither exists — meaningful locally, honest in CI, never a false failure. **Lesson:** any test shelling `git diff <ref>` must resolve the ref or skip; never assume the local branch name exists in CI. Caught only because the full suite green was verified at the PR gate, not just locally.
