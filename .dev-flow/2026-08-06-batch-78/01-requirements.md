...
| **Inc-2** | **HLR-122** — selectable run list + the C-38 sweep + **the `test_tc029` rewrite (Q-M2)** | `AT-B78-15…19` | `screens_directionb.py`, `styles.tcss`, `tests/test_tui_diff_screen.py`, `tests/test_tui_diff_compare_realpath.py` (4) | C-34 + full `test_tui_diff_compare_realpath.py` + full `test_tui_patch_big.py` (C-26, 25 refs / 5 files); `TC-011` green; **`test_tc029` reddens under `cap 128 → 100000` after the rewrite** |
