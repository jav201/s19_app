...
- **Measured values:** `_DIFF_WIDE_MIN = 139` **terminal columns** — the first terminal width at which a 22-**content**-cell list leaves an unwrapped window under `L ≤ C − 89` (confirmed 138 → 78 ✗, 139 → 79 ✅). `_DIFF_MIN_W = 94` **terminal columns** (93 → 78 ✗, 94 → 79 ✅). ~~`_DIFF_MIN_H = 29`~~ → **`_DIFF_MIN_H = 26` terminal rows** — **`A-1` DISCHARGED at Inc-5 by re-derivation against the real implementation** (the original 29 came from simulating S-8 with `styles.height = 1`). Measured with the fallback layout forced: `25 → 0, 26 → 1, 27 → 1, 28 → 2`, identically at `W = 94`, `120` and `138` — **width-independent, so the spec's claim reproduced exactly and only the value moved.** The 29 is reproducible on this branch as the floor with the command-bar row **still present**, which is not the state this quantity is defined in (*post-US-78-8 + US-78-1*). ⚠️ **The first sweep was CIRCULAR and was discarded:** the constant selected which regime it measured, so every height below it read 0 for a reason that *was* the constant (`C-78-xxiv`). ⚠️ **26 is the post-Inc-10 floor:** between now and Inc-10 the heights 26–28 select the fallback regime while painting no hex row — deliberate, since Inc-10's file set cannot change the constant, and stated in the constant's own docstring. **120×30 is unaffected.**
