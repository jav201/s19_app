...
- **Measured values:** `_DIFF_WIDE_MIN = 139` **terminal columns** — the first terminal width at which a 22-**content**-cell list leaves an unwrapped window under `L ≤ C − 89` (confirmed 138 → 78 ✗, 139 → 79 ✅). `_DIFF_MIN_W = 94` **terminal columns** (93 → 78 ✗, 94 → 79 ✅). `_DIFF_MIN_H = 29` **terminal rows** (28 → 0, 29 → 1) — **`assumed — re-derive at Phase 3`** against the real S-8 implementation, since the measurement simulated it with `styles.height = 1`.
