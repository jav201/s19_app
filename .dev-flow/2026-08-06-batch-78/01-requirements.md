# Requirements Document — s19_app — batch `2026-08-06-batch-78` (MERGED, Phase-1 FOLD — **revision 2**)
...
**Revision 2** answers Phase-2's BLOCK: architect 3 blocker / 11 major / 8 minor · QA 1 blocker / 6 major / 8 minor · security 0 blocker / 1 major / 1 minor. Every fix below carries an executed transcript. **The measured foundation was independently re-executed by two reviewer lanes and every load-bearing figure reproduced** — including 79 cells, `W ≥ 94` and the `assumed` `H = 29`, which the architect lane confirmed at three widths where this document had two. Scope, story set, regime design and lane ordering are unchanged.
...
| **painted child** | A Footer child is *painted* **iff** `child.region.x >= footer.region.x` **and** `child.region.x + child.region.width <= footer.region.x + footer.size.width` — i.e. containment evaluated **entirely in absolute (screen) coordinates**, using the Footer's absolute right edge `region.x + size.width`. ⚠️ **The coordinate space is normative because three parties produced three counts from it.** `Footer.region.x == 1` (executed), so comparing an **absolute** child `x + width` against the **relative** `size.width` mixes frames and silently discards the right-aligned child that ends flush with the edge. This is the same **units** defect as `_DIFF_WIDE_MIN`'s content-vs-terminal confusion (M6): *a bare figure with no stated measurement convention is not re-derivable.* |
...
**Lane 2.** `AbDiffPanel` (`screens_directionb.py:6566`) composes three 3-row control rows, a status line, and `#diff_columns` (`:6769`) — three rigid `1fr` boxes (`styles.tcss:1481-1490`). `render_comparison` (`:6877`) caps runs (`_apply_display_caps`, `:6926`), renders the list into a `Static` (`_render_run_list`, `:6958`), and renders **only run 0** (`_render_run_windows(0)`, single call site `:6921`).
...
| C7 | Diff result area, `#diff_hex_a` — **every figure labelled with its layer** (M1/Q-m7) | **content** rows: 80×24 → **0** · 120×30 → **0** · 132×44 → **7** · 160×40 → **3**. **clipped** rows: 0 · 0 · **11** · **7**. For these bordered boxes `clipped = content + 4` (2 border + 2 padding) | measured (P-33) |
...
| C16 | Footer chip budget — **two different quantities, layer-labelled** | **model:** `active_bindings` with `show and enabled` = **14**, identical at every size. **painted (§1.3's absolute-coordinate containment rule):** **8** @80×24 · **13** @120×30 · **15** @160×40, out of 15 children | executed at three sizes (R-5) |
...
| **P-37** | Deleting `test_tui_commandbar.py` nodes is registry-neutral | hypothesis | ❌ **FALSE — G2 goes red** | 4 `LIVE` rows name it: `TC-007`, `TC-008`, `TC-009`, `TC-039`; `TC-008` alone names 5 such nodes. G2 = *"every LIVE entry's nodes all exist"* (`tests/test_id_registry.py:222`). | Widens US-78-4 by one file. |
| **P-38** | `#cmdbar_project` has 1 test consumer | premise | ❌ **FALSE — 3 helpers, 14 call sites, 2 files Phase 0 never named** | `tests/test_tui_patch_variant.py:85` (helper `_project_label` `:82`, **9** calls) · `tests/test_tui_variants.py:78` (**6** calls) · `tests/test_tui_directionb.py:897`. | **US-78-3 must land BEFORE US-78-1.** |
...
| **P-38c** | The project label has one display form | hypothesis | ❌ **FALSE — two, and the single-variant one is pinned** | `app.py:11328` plain default; the suffixed form is gated on `len(variant_set.variants) > 1` (`:11333`) → `f"{p}:{d} ({i}/{N})"` (`:11345-11348`). `tests/test_tui_variants.py:259` `test_single_s19_project_label_plain` pins *"no `(1/1)`"* under LLR-005.3. | **Corrected my own phantom `(1/1)`** (F-4 in §6.3). Drives LLR-120.5. |
...
> **Height:** the window paints at least one **HEX row** — measured floor **`H ≥ 28`** post-US-78-8 + US-78-1 (27 → 0 hex, 28 → 1, 30 → 2), width-independent.
>
> ⛔ **CORRECTED AT THE Inc-5 GATE, twice, and the wording is the whole point.** This clause originally read *"at least one visible **content** row"* with a floor of **29** measured against a simulation; re-derived against the real implementation under that same wording it gave **26**. **Both were wrong, because a painted content row can be the window's own header and buys the operator zero bytes.** Normative **`LLR-125.2`** already said *"at least one **hex row** of content"* → **28**, so this was the weaker of two clauses in one document, and the implementation took it without flagging the conflict. **`LLR-125.2` governs**; the metric is corrected here rather than left to contradict §5.6.1. At 26–27 the panel would have declared the terminal usable, **hidden the notice**, and painted a header and nothing else — **strictly worse than 25**, where the operator is at least told why.
...
| **Fallback** | `94 ≤ W < 139` **and** `H ≥ _DIFF_MIN_H` | run list as a **dismissible overlay** (dedicated key); window at full `C`; the visible hex rows are a **paginable viewport** over the selected run — at 120×30 that is **2 rows**, accepted explicitly by the operator |
| **Notice** | `W < 94` **or** `H < _DIFF_MIN_H` | an explicit notice naming **every** unsatisfied axis and the value each requires, instead of a silently empty panel. **80×24 fails BOTH**, and it is the only case `AT-B78-29` tests (M7) |
...
⚠️ **Reported back to the coordinator, as asked:** R-1's premises hold under measurement, and both reviewer lanes re-executed the ladder independently — 120×30 → 0/0/**2** content reproduces to the digit. The refinements measurement forces are the *axis-naming* clause above (**every** axis, because 80×24 fails both) and SEC-F2's bound on the notice text. `_DIFF_MIN_H = 29` is `assumed — re-derive at Phase 3` against the real S-8 implementation, since my simulation set `styles.height = 1` directly.
...
> **No global `AT-NNN`/`TC-NNN` is minted.** Batch-scoped `AT-B78-01…33` (`AT-B78-27` **retired**, §5.6.2) / `TC-B78-01…48`.
...
- **Acceptance (black-box):**
  ...
  - **Deliverable + observation:** the rendered widget tree per screen; the slot's height; the palette's action set compared against **the Inc-0 artifact re-read from disk** (the C-12 output-then-consume shape already specified for `AT-B78-12`) — **never against `app._build_palette_entries()`**. ⚠️ **BL-1, executed:** `CommandBar` is constructed as `CommandBar(self._build_palette_entries())` (`app.py:1878`) and `visible_palette_actions()` returns that same tuple (`command_bar.py:204-205`, `:137`), so a producer-derived expectation is `f(x) == f(x)`. Removing `Binding(question_mark → show_help_panel)` from `S19TuiApp.BINDINGS` left the predicate **GREEN at 36 == 36**: `PRE (True, 37, 37) → MUT (True, 36, 36) → POST (True, 37, 37)`. **C-31 is misapplied to an invariance claim: the "rule" here is the pre-change capture, not the live producer.** ⚠️ **C-40:** absence is trivially green on a screen that mounted nothing — the AT co-asserts, in the same run, that `#command_palette` resolves **and that its parent is the `CommandBar` instance** (the LLR-118.1 position invariant, restored per the reverse audit §5.6).
  - **Acceptance test(s):** **`AT-B78-01`** (GATE) row + child ids absent on all 10, slot height 0, palette present **and still a direct child of `CommandBar`** · **`AT-B78-02`** (**PIN**) Ctrl+K opens on all 10, blur asserted · **`AT-B78-03`** (**PIN**) the action set equals the **Inc-0 artifact**, re-read from disk
...
- **Rationale (informative):** executed on all 10 screens: `/` → `find_input`, `g` → `cmdbar_goto_input`, `escape` → **still `find_input`**. Because `action_show_screen` swaps a `hidden` class and nothing unmounts (Phase-0 P-10), all eight inputs resolve on every screen — routing keys on `_active_screen_key`, never on presence (the Statement above is the normative form). The defect is real and executes: with an image loaded and **A2L** active, a find writes `'BOOT'` into the **workspace** `#search_input` and the search actually runs against the workspace map. The notice path is the **majority** — 7 of 10 screens.
...
- **Acceptance (black-box):**
  ...
  - **Deliverable + observation:** `app.focused.id`; each local `Input.value`; `#log_line_4` and the `len(app.log_lines)` delta.
  - **⚠️ The `OWNING` derivation is stated normatively, because two honest derivations disagree (Q-M3, executed).** A screen owns find/goto **iff** `app.query_one(f"#{container}").query(Input)` yields at least one id matching the regex `(search|goto)_input$`. Executed over `SCREEN_CONTAINER_IDS`:

    | derivation | result |
    |---|---|
    | *any* `Input` in the container | **n=7** — `a2l, crc_designer, diff, flow, mac, patch, workspace` |
    | id matches `(search\|goto)_input$` | **n=3** — `a2l, mac, workspace` |

    Per-container `Input` counts: `workspace 2 · a2l 4 · mac 2 · patch 5 · diff 3 · flow 1 · crc_designer 13`. **The obvious derivation gives 7, so a bare `assert len(OWNING) == 3` false-fails a faithful implementation** — and any derivation yielding 3 without the stated pattern is `LLR-119.1`'s own map inverted, i.e. both partitions would come from the artefact under test (C-40 limb 2). The AT therefore asserts **set equality** `OWNING == {workspace, a2l, mac}` **and** `OWNING ∪ NOTICE == SCREEN_CONTAINER_IDS.keys()`; `len(OWNING) == 3` is kept only as a redundant guard. *An unstated grep pattern is an unstated definition.*
...
- **Numeric pass threshold:** with a project and A2L loaded, the status bar's text contains both on **10 of 10** screens (today **0 of 10**); `#loaded_panel` contains the project name (today absent); a 1-variant project's string contains **no** `(`; a 2-variant project's contains `(1/2)`; `1 <= #workspace_status_bar.size.height <= 7`
- **Priority:** high
- **Acceptance (black-box):**
  ...
  - **Acceptance test(s):** **`AT-B78-08`** (GATE) status bar names both, all 10 screens · **`AT-B78-09`** (GATE) `#loaded_panel` names the **project** (the A2L clause is dropped — it is green today) · **`AT-B78-10`** (**PIN** until the successor exists, then GATE) the update **path**: after a variant switch **both** surfaces move, asserted `before != after` per surface **and** `after` equal to the expected string computed from the variant set · **`AT-B78-11`** (**PIN**, green today) the status bar's height does not grow **and does not collapse** — `1 <= height <= 7`, asserted in the same run as `AT-B78-08` so a height-0 bar cannot pass both (NEW-7) · **`AT-B78-30`** (GATE) the display-form arm: plain at `N == 1`, `(1/2)` at `N == 2`, **on both surfaces** — four observations
  - **Boundary catalog:** ☑ **empty** — nothing loaded → both surfaces show the `(none)` sentinels, not a blank, `TC-B78-09` · ☑ **alternative** — A2L loaded with **no** project, `TC-B78-10` · ☑ **boundary** — `N == 1` renders the **plain** name and no counter (⚠️ an earlier draft of mine asserted `(1/1)`; `app.py:11333` gates the suffix on `> 1` and `tests/test_tui_variants.py:259` forbids it — the literal would have false-failed a correct implementation *and* contradicted a shipped acceptance), `TC-B78-11` · ☑ **boundary** — `N == 3`, active index 2 → `(2/3)`, `TC-B78-42` · ☑ **invalid — the control-character limb (SEC-F1)** — a filename carrying **both** hostile shapes, `evil\x1b[31m\x9b1m\x9d8;;http:x\x7f[red].a2l`, renders with **no `MarkupError`** *and* with **no codepoint of `C0 ∪ {DEL} ∪ C1` in the painted strip**, on **both** surfaces, `TC-B78-12` · ☑ **negative** — unload all → both surfaces return to `(none)`, `TC-B78-43` · ☑ **error** — `update_project_labels` before mount → no raise, `TC-B78-13`
...
- **Numeric pass threshold:** a **class-qualified** AST census finds **0** definitions of the **seven** symbols `CommandBar.Find`, `CommandBar.Goto`, `on_command_bar_find`, `on_command_bar_goto`, `CommandBar.focus_find`, `CommandBar.focus_goto`, `CommandBar.set_context_labels` (executed today: **1 each, total 7**); `styles.tcss` contains **0** selectors for the six deleted ids and **retains** `#command_bar_slot` and `#command_bar`; the 9-row behaviour payload re-read from disk equals a fresh live capture; `tests/test_id_registry.py` G1–G7 green
- **Priority:** medium
- **Acceptance (black-box):**
...
- **Acceptance (black-box):**
  ...
  - **⚠️ `AT-B78-22` was a live sibling of F-6 and is rewritten (Q-M1, executed).** Its expected span was computed from `AbDiffPanel.DISPLAY_CONTEXT_BYTES` — the class under test — so mutating `16 → 64` left it **GREEN**. The rewrite fixes the expectation to **literal addresses** and puts the constant in a **guard only**.
  - **⚠️ NEW-3 — the discharging mutation had to change too.** The guard is an *equality* on that constant, so `ctx 16 → 64` raises **in the guard** in **both** placements and never reaches the assertion (executed: `guard-FIRST = GUARD-RAISED`, `guard-AFTER = GUARD-RAISED`). *A counterfactual must fail on its assertion, not on an error* — the lesson Q-m2 taught the cap AT, unapplied to the AT rewritten in the same revision. **The mutation is therefore on the IMPLEMENTATION, with the guard left true:** `screens_directionb.py:7028` `high = end + self.DISPLAY_CONTEXT_BYTES` → `high = end`. (⚠️ For the record: the alignment step **does** exist, at `screens_directionb.py:7027`; the architect lane's round-1 claim to the contrary came from reading `:7026`, `:7028` and `:7029` and skipping it, and it corrected itself at the re-gate.) Executed: `PRE ('green', ['00000FF0','00001000','00001010']) → MUT ('RED-on-assertion', ['00000FF0','00001000']) → POST ('green', …)`.
  - ⚠️ **The other proposed mutation does NOT work, and is recorded so it is not retried.** `low -= low % HEX_WIDTH → low -= 0` (proposed by the **QA lane**) leaves the predicate **green**. My first reason was fixture-specific — the run starts at `0x1000`, so `low = 0xFF0` and `0xFF0 % 16 == 0`. **The architect lane then gave the general form, which is stronger and is the one to record:** `DISPLAY_CONTEXT_BYTES == HEX_WIDTH == 16`, so `low = start - 16 ≡ start (mod 16)` — the alignment step is a no-op for **every** 16-aligned run start, not merely this fixture's. **A mutation must be executed against the actual fixture, and its inertness explained in the general form, not the particular one.**
  - **Acceptance test(s):** **`AT-B78-20`** (GATE) selecting run 3 re-renders both windows, asserting the **address** and the first data row's position, not merely "changed" · **`AT-B78-21`** (GATE) the row count differs between two pane heights and each is ≤ its **content** height · **`AT-B78-22`** (GATE, rewritten) the emitted row addresses equal the three literals above, with the constant only in a guard
  - **Boundary catalog:** ☑ **empty** — 0 runs → the "no differing runs" text stays, `TC-B78-23` · ☑ **boundary** — a run at address 0, exercising the `max(0, …)` clamp at `:7026`, **and a second arm with an UNALIGNED start (e.g. `0x1007`) so the alignment step at `:7027` (`low -= low % HEX_WIDTH`) is exercised by an acceptance** — without it that line is covered by nothing, which is why the mutation on it came back inert (architect NEW-11), `TC-B78-24` · ☑ **boundary** — a run longer than the pane: paginable, header still names the run, `TC-B78-25` · ☑ **boundary** — a run whose bytes are absent from one map (the blank-gutter form, executed), `TC-B78-26` · ☑ **invalid** — selection index outside `_runs` → the existing early return at `:7024`, `TC-B78-27` · ☑ **error** — a re-render while the pane has zero height → no raise, `TC-B78-28` · ☑ **mechanism** — the emitted row count is no longer a pure function of `DISPLAY_CONTEXT_BYTES` (two pane heights, one constant), `TC-B78-45`

---

### HLR-124 — three width/height regimes, and no silently empty panel
- **Traceability:** US-78-7 · **operator ruling R-1**
- **Statement:** When the terminal width is at least the wide-regime breakpoint, `#diff_columns` **shall** render the run list alongside the hex-window column; when the terminal satisfies the deliverability condition but not the wide breakpoint, `#diff_columns` **shall** render the hex-window column at its full content width, **shall** present the run list without permanently reserving columns or rows from that width, and **shall** make the selected run's remaining bytes reachable by pagination within the visible rows; when the terminal does not satisfy the deliverability condition, the panel **shall** render a notice naming **every** unsatisfied axis and the value each requires; and in the first two regimes each hex window's content width **shall** be at least the width of the widest row `hexview.render_hex_view` emits.
- **Rationale (informative):** the shipped three-`1fr` split gives each window **26** cells at 120 and ~**42** at 160 against a **79**-cell row — every hex row wraps at every supported width. The regime boundaries are derived, not chosen (D-1): width `W ≥ 94` (measured exact, 93 → 78 ✗, 94 → 79 ✅) and height `H ≥ _DIFF_MIN_H` (**FINAL 28 — re-derived twice at Inc-5; the intermediate 26 measured PAINTED rows where the deliverable is HEX rows, and 25 → 0, 26 → 1; the drafted 28 → 0 / 29 → 1 was measured with the command bar still present, and by a sweep the constant itself could veto — see §5.6.1**). The wide breakpoint **139** is the first fit for a 22-cell list under `L ≤ C − 89`, confirmed to the column (138 → 78 ✗, 139 → 79 ✅). **80×24 fails both axes** and is the reason the notice regime exists: today that panel renders nothing and says nothing about why.
...
- **Numeric pass threshold:** at **160×40** and at **132×44**, `#diff_hex_a.size.width >= max(len(line) for line in render_hex_view(...).splitlines())` — today `42 >= 79` **False** and `30 >= 79` **False**; at **120×30** the same holds post-US-78-8/US-78-1; at **80×24** the notice is present and names **BOTH** axes (80×24 fails width *and* height — it is the only case this AT tests); the literals `139`, `94` and `29` appear **0** times in **the new `AT-B78-*` / `TC-B78-*` nodes** (⚠️ unscoped, this threshold is **already false on `origin/main`** — executed: `tests/test_tui_directionb.py` contains `94`×1 and `29`×4; `26` is dropped as a sentinel because any address or size can produce it)
- **Priority:** high
- **Acceptance (black-box):**
  ...
  - **Deliverable + observation:** the window's **content width** against the **emitted** row width, which the test derives by **calling `render_hex_view` itself at test time**; plus the clipped visible **width and height**, so the window is not merely un-wrapped but actually on screen. ⚠️ **M3:** a clipped-*width*-only clause is invariant under total vertical invisibility — at 120×30 shipped, `#diff_hex_a`'s clipped region is **(width 30, height 0)**. The **height ≥ 1** co-assertion is what makes the clause do its job. ⚠️ **C-36/C-39:** the test does **not** hard-code `79` — it is a measurement, not a constant on disk, and a literal would be a phantom the day `HEX_WIDTH` changes. Both quantities are computed. ⚠️ **Per-arm verdicts (CC-1):** every arm is reported per resolved node id per size; 120×30 and 132×44 differ by 11 pane rows and a single aggregate verdict would destroy exactly the information this requirement rests on.
  - **Acceptance test(s):** **`AT-B78-23`** (GATE) no wrapped row at 160×40 · **`AT-B78-24`** (GATE) no wrapped row at 132×44 and, post-US-78-8/1, at 120×30 · **`AT-B78-25`** (GATE) the regimes are observably different: at 160 the list is visible beside the window; at 120 it reserves **0** permanent columns and the viewport paginates · **`AT-B78-29`** (GATE) at 80×24 the notice is present and names **every** unsatisfied axis — at 80×24 that is **both**
  - **Boundary catalog:** ☑ **boundary** — one column below the wide breakpoint and one at it: the layout flips exactly once, `TC-B78-29` · ☑ **boundary** — a resize **across** the breakpoint after a comparison is rendered: the layout follows and the windows do not blank, `TC-B78-30` · ☑ **boundary** — `size=(_DIFF_MIN_W, …)` and `(_DIFF_MIN_W - 1, …)`: the notice appears exactly once, at the second, `TC-B78-31` · ☑ **boundary** — a terminal passing width and failing height (e.g. 120×24) → the notice names the **height** axis, not the width one, `TC-B78-32` · ☑ **empty** — no comparison rendered: the regime still applies, nothing raises, `TC-B78-33` · ☐ **error** — **N/A:** layout selection consumes only the terminal size, already validated by Textual.
...
- **⚠️ C-13/C-23:** the 1-row target is not transferred from another container; it is the residual required to satisfy the second clause. Phase 3 re-measures the whole `#ab_diff_panel` budget — every sibling — not just these three rows.
...
- **Acceptance (black-box):**
  ...
  - **Acceptance test(s):** **`AT-B78-26`** (GATE, **one node**) the three rows clip to 1, `#diff_status` is visible, **and** at 120×30 with the bar removed `#diff_hex_a` renders ≥ 1 hex row — all in the same test, landing whole at **Inc-10** · **`AT-B78-33`** (GATE) **Inc-1's own gate**: at 132×44 compaction alone raises `#diff_hex_a.size.height` above the pre-change baseline of 7 (executed: **13**)
  - **⚠️ Q-M6.1 / BL-3 — rev-1 split this pair across Inc-1 and Inc-9 while requiring it to be one test.** Two tests in two increments cannot be one test; and `AT-B78-26`'s row-height clause alone is **green with the result area still at zero** — the precise state LLR-125.2 exists to prevent. `AT-B78-27` is therefore **retired into `AT-B78-26`** (§5.6.2), and Inc-1 gets `AT-B78-33`, which is observable at 132×44 with no Lane-1 work.
...
- **Numeric pass threshold:** with the list focused the help panel names its navigation keys; the list's rendered text contains the affordance; the `show and enabled` key set is **set-equal** to the committed pre-change set of **14** (a *containment* predicate is satisfied by a superset — executed: `+1 chip → contains-all True, set-equality False`); and the **painted** child count under §1.3's containment rule is unchanged **per size** — **8** @80×24 · **13** @120×30 · **15** @160×40, reported as three CC-1 arms (§5.1 rule 7), with **160×40 as the load-bearing arm**; `git diff` over `app.py:1338-1375` is **0** lines
- **Priority:** low
- **Acceptance (black-box):**
...
- **Acceptance criteria:** the map's key set is asserted **set-equal to `SCREEN_CONTAINER_IDS.keys()`** — the set comes from the RULE, not the implementation's own list (C-40 limb 2). `TC-B78-46` asserts that with A2L active `#search_input` is **still resolvable** while the action resolves the A2L input — the white-box discrimination P-10 demands. (M5: rev-1 loaded this onto `TC-B78-41`, which already carried the pre-mount error case. One id, two nodes, in a project with a node-level registry guard.)
...
- **⚠️ C-16:** the mechanism (an Input-scoped binding, an `on_key`, or a screen-level priority binding) is `assumed — verify in target framework at Phase 3`.
- ❌ **The "must not shadow the palette's `escape`-to-close" constraint rests on a FALSE PREMISE, executed at Inc-5.** **The palette has no `escape`-to-close at all:** `command_bar.py` declares **no `BINDINGS`** and handles **no `escape`**; executed, `ctrl+k` then `escape` leaves `palette_is_open` **True**. There was nothing to shadow. Both lanes' probes were read as exercising a mechanism that does not exist — a claim two independent reviewers accepted because it *sounded* like a property the widget would obviously have. **Inc-9 must not assume the mechanism exists.** Two dispositions are open to Inc-9 and it must choose in writing rather than inherit: treat the absence as out of scope (the bar is being deleted anyway, so the palette's dismissal is `HLR-118`'s business), or close it deliberately. **What is forbidden is asserting the constraint against nothing**, which is what this clause did.
...
- **Validation:** `test (integration)` · **Numeric pass threshold:** `1 <= #workspace_status_bar.size.height <= 7` on all 10 screens after the change (today **7**). ⚠️ `== 7` would false-fail a conforming implementation that *reduces* the height (Q-m6); a bare `<= 7` would be satisfied by a **collapsed** bar of height 0 (NEW-7), so the floor is normative, and `AT-B78-11` is asserted **in the same run as `AT-B78-08`** so a collapsed bar cannot pass both

**LLR-120.4 — the new sinks are markup- AND control-character-safe**
- **Statement:** The project string and the A2L filename **shall** be rendered through `safe_text` on both context surfaces.
- **Symbols:** `safe_text` `screens_directionb.py:878` — *"Delete every C0/C1 control codepoint from `value` (`_CONTROL_SCRUB`, LLR-116.7), then wrap the remainder as a `Text`"* (`:882-884`); the existing panel precedent `_build_slot_row` `screens_directionb.py:1984-1987`, whose docstring already reads *"File-derived name (escaped here via `safe_text`)"*; the weaker precedent `Label("Ready.", id="status_text", markup=False)` `app.py:1916`.
- **⚠️ Rationale (informative) — SEC-F1, and the disjunction this LLR used to carry was wrong.** `markup=False` is a **parse flag that performs no string transform**. Executed on the payload `b78probe\x1b[31m_evil[red]\x9b1m\x9d8;;file:///etc/passwd\x07\x00\x7f`:

  | control | `markup=False` | `safe_text` |
  |---|---|---|
  | surviving codepoints | `0x0 0x7 0x1b 0x7f 0x9b 0x9d` — **byte-identical to the payload** | **none**; output `'b78probe[31m_evil[red]1m8;;file:///etc/passwd'` |

  `U+009B` (single-byte C1 CSI) and `U+009D` (single-byte C1 OSC) carry **no `\x1b` at all** and are **legal Windows filename characters**, so an SGR recolour or an OSC-8 clickable hyperlink is reachable through a hostile A2L filename or project directory name on the operator's own platform. **`#loaded_panel` is where this is a genuine regression:** every existing file-derived cell there already goes through `safe_text`, so a `markup=False`-only project row would sit scrub-inconsistent beside scrubbed siblings — and `grep -rl "loaded_slots\|_build_slot_row" tests/` returns **0 files**, so nothing in the repo would catch it.
- **Validation:** `test (integration)` + `inspection` · **Numeric pass threshold:** the painted strip of both surfaces contains **no** codepoint in `C0 ∪ {DEL} ∪ C1` and raises no `MarkupError`
- **Acceptance criteria:** ⚠️ `TC-B78-12`'s payload **must carry control characters**. Its previous payload `[red]evil[/].a2l` contains **zero** (executed) and is therefore **GREEN under both arms of the retired disjunction** — it certifies the markup axis and is blind to the control axis, the same shape as this batch's own F-6. The control class is **re-derived in the test**, never imported from `_CONTROL_SCRUB` (the discipline `tests/test_tui_hostile_map.py:86-89` already states).
...
- **Symbols:** the branch producing the two forms — `app.py:11328` (plain) and `:11329-11348` (suffixed, gated `> 1` at `:11333`); `_variant_display_options` `:11338`; the live pin `tests/test_tui_variants.py:259` `test_single_s19_project_label_plain` (*"no `(1/1)`"*, `:260`, LLR-005.3); the reading helpers `tests/test_tui_variants.py:76-78` (**6** calls) and `tests/test_tui_patch_variant.py:82-85` (**8** calls).
...
- **⚠️ Digest recipe (Q-m3) — rev-1 recorded a hash triple with no way to re-derive it.** The payload row is `(screen, query, goto, log_line_4_after_search, last_search_address, log_line_4_after_goto, per_view_goto_focus_address)`; the artifact is the 9 rows serialised as sorted-key JSON with `ensure_ascii=True`, and the digest is `blake2b(digest_size=8)` over its UTF-8 bytes. **The digest is a convenience, not the oracle — the oracle is the row-by-row comparison.** ⚠️ `find_string_in_mem` is the module-level import used by all three `_handle_search*` paths (`app.py:11468`, `:11559`, `:11609`), so the recorded mutation reaches only the **search** half; a second mutation on `_handle_goto`'s address parse is **owed at Inc-11** to cover the go-to half.
...
- **Symbols:** `TC-007`, `TC-008`, `TC-009`, `TC-039` — the 4 rows matching `test_tui_commandbar`; `TC-008` names 5 such nodes. Guard `tests/test_id_registry.py:222`, registered `:361`, asserted `:388-390`.
...
- **Validation:** `inspection` + `test` · **Numeric pass threshold:** `grep -rn 'cmdbar_project' tests/` → **0** after the sequence (today **3** definitions, 14 call sites)

### HLR-122 → LLR-122.x

**LLR-122.1 — the run list becomes a selectable widget**
- **Statement:** `#diff_range_list` **shall** be a widget maintaining a selection index over one entry per displayed run, replacing the `Static` that renders the runs as text, and the artifact-usage summaries it renders **shall** remain markup-inert — either escaped as today or rendered `markup=False`.
- **Symbols:** `Static("Runs", id="diff_range_list", markup=True)` `screens_directionb.py:6766`; `_render_run_list` `:6958`, terminal write `:7001`; the caps-notice branch `:6995-6999`; CSS `styles.tcss:1481-1490`.
...
- **⚠️ C-17 (M10) — rev-1 carried this as a normative clause inside a note, with no AT and no TC. It is now in the Statement above and carries `TC-B78-48`.** The list renders `markup=True` and escapes the artifact summaries with `rich.markup.escape` (imported `:6980`, applied `:6984-6985`); **the widget-type swap is exactly where that escape gets lost.** The security lane established the residual risk is low — the summaries resolve to one of four module constants (`compare_service.py:54-58`) — so the clause is precautionary; but a precautionary clause with no node is not a control.
...
- **Acceptance criteria — RESTORED at revision 2 (§5.6.1; the loss had a consequence):** `pilot.press` only, with the LLR-119.4 blur discipline in its **mirror-image** form — `AT-B78-19` asserts `app.focused is <the run list>` as an explicit precondition before pressing. Executed on the current tree, `set_focus` on the `Static` run list **silently does not take** (focus lands on `RailItem(id='rail_item_workspace')`) and `k` opens the Legend anyway, so rev-1's predicate was **green with its precondition false** — green for a reason with nothing to do with its subject.
...
- **Symbols:** `_apply_display_caps` `screens_directionb.py:6926`; `DISPLAY_MAX_RUNS = 128` `:6623`; `DISPLAY_MAX_TOTAL_BYTES` `:6624`; the notice branch `:6995-6999`.
- **Acceptance criteria:** ⚠️ **the predicate does not read `DISPLAY_MAX_RUNS` for its expected value.** QA's first form fed `DISPLAY_MAX_RUNS + 5` runs and expected `f"showing {DISPLAY_MAX_RUNS} of {n}"` — it stayed **GREEN under `128 → 100000`** because the fixture and the expectation moved together. The rewritten form fixes the fixture at **200** independently, guards with `assert 200 > DISPLAY_MAX_RUNS`, and asserts only the **painted** row count and the notice's own two numbers. **C-39's "quote the constant, never its value" applies to the *guard*, not to the *expectation*.** ⚠️ **Guard placement (Q-m2):** the guard is evaluated **after** the capture, so the mutation reddens the **assertion** rather than raising in the guard — a counterfactual must fail on its assertion, not on an error. Both transcripts are recorded in §5.3.
- **⚠️ The same defect is ALREADY ON DISK, in the file Inc-2 edits (Q-M2, executed).** `tests/test_tui_diff_screen.py:284` `test_tc029_display_caps_bound_on_screen_runs` — a **LIVE registry node** — reads the constant on both sides (`over = DISPLAY_MAX_RUNS + 50`; `assert n_displayed <= DISPLAY_MAX_RUNS`). Re-executed here with the same in-process mutation:

  ```
  PRE  cap=128    -> (assertion True, over=178,    stored=128)
  MUT  cap=100000 -> (assertion True, over=100050, stored=100000)   <- GREEN == INERT
  POST cap=128    -> (assertion True, over=178,    stored=128)
  ```

  **Disposition (required, not deferred):** `Inc-2` rewrites `test_tc029` to the `AT-B78-18` form in the same increment that hardens the clause. Leaving it would land a hardened `AT-B78-18` **beside an inert guard on the same clause in the same file**, and a future reader would reasonably take the older node as coverage. `LLR-125.1`'s "check for duplication before minting a new node" points an implementer straight at it. Its registry row keeps its id; only the node body changes, so **G2 is unaffected**.
...
- **Symbols:** `_render_run_windows` `screens_directionb.py:7003`, **single** call site `:6921` with the literal `0`, inside `render_comparison` (`:6877`); the phantom handler its docstring cites (`on_data_table_row_selected`, `:7019`) has **0** definitions in the module.
...
- **Statement:** The number of `row_bases` passed to `render_hex_view` **shall** be computed from the window widget's rendered height at render time, bounded below by the run's bytes plus `DISPLAY_CONTEXT_BYTES` each side and above by the window's **content** height (`size.height`), with the clipped height asserted separately as the visibility co-assertion.
...
- **Validation:** `test (integration)` · **Numeric pass threshold:** row counts at two pane heights **differ** (today 4 and 4) and each is **≤** its **content** height (`size.height`). ⚠️ `≤ clipped` admits **four rows of invisible overflow** — for these bordered boxes `clipped = content + 4` whenever the widget is inside its host (132×44: 11 vs 7; 160×40: 7 vs 3). The clipped read stays as the separate visibility co-assertion, never as the bound
...
- **Symbols:** current header `f"Run #{run_index} 0x{start:08X}-0x{end:08X}"` `:7032` — names index and range but **not** kind; `_KIND_LABEL` `screens_directionb.py:6637-6641` (a 3-entry constant dict; **re-derived this session — the earlier `assumed` flag is withdrawn**).
...
- **Acceptance criteria:** ⚠️ **C-17:** `#diff_hex_a`/`#diff_hex_b` are constructed `markup=False` (`:6767-6768`) and remain so. The header composes only integers and constant labels — B3 holds by construction.
...
- **Measured values:** `_DIFF_WIDE_MIN = 139` **terminal columns** — the first terminal width at which a 22-**content**-cell list leaves an unwrapped window under `L ≤ C − 89` (confirmed 138 → 78 ✗, 139 → 79 ✅). `_DIFF_MIN_W = 94` **terminal columns** (93 → 78 ✗, 94 → 79 ✅). ~~`_DIFF_MIN_H = 29`~~ → ~~`26`~~ → **`_DIFF_MIN_H = 28` terminal rows** *(final — see the ⛔ block below; 26 was itself wrong)* — **`A-1` DISCHARGED at Inc-5 by re-derivation against the real implementation** (the original 29 came from simulating S-8 with `styles.height = 1`). Measured with the fallback layout forced: `25 → 0, 26 → 1, 27 → 1, 28 → 2`, identically at `W = 94`, `120` and `138` — **width-independent, so the spec's claim reproduced exactly and only the value moved.** The 29 is reproducible on this branch as the floor with the command-bar row **still present**, which is not the state this quantity is defined in (*post-US-78-8 + US-78-1*). ⚠️ **The first sweep was CIRCULAR and was discarded:** the constant selected which regime it measured, so every height below it read 0 for a reason that *was* the constant (`C-78-xxiv`). ⛔ **26 WAS ALSO WRONG, and the Inc-5 gate caught it. The floor is 28.** The re-derivation measured *painted content rows*, but **the deliverable is HEX rows**, and the two differ by the header line the window renders itself — `_window_row_capacity` is `min(A, B) − 1` **because of that header**, written by the same author one increment earlier.

| H | capacity | painted | **visible hex rows** |
|---|---|---|---|
| 26 | 0 | 1 | **0** ← would have shipped as "deliverable" |
| 27 | 0 | 1 | **0** ← would have shipped as "deliverable" |
| **28** | 1 | 2 | **1** ← the floor |
| 30 | 2 | 3 | **2** — R-1's accepted viewport, to the row |

Identical at `W = 94, 120, 138`. At 26–27 the panel would declare the terminal usable, **hide the notice**, and paint a header and nothing else — **strictly worse than 25**, where the operator is at least told why. A regime worse than the one below it is not a degradation ladder.

**The root cause was a live spec conflict resolved silently:** `D-1`'s *"at least one **visible content row**"* → 26 against normative **`LLR-125.2`**'s *"at least one **hex row** of content"* → **28**. **`LLR-125.2` governs.** `D-1`'s height axis is corrected in §2.8 rather than left to contradict this line, and the constant's docstring **names both clauses and says which wins** instead of averaging them.

**`TC-B78-54` is the node that was missing** — the height floor asserted on **both sides**: below it the notice is present and names the height axis; at it the notice is absent **and at least one hex row is painted**. **G1 is the proof it is the right control: restoring `26` reddens `TC-B78-54` ALONE — even though the node derives its sizes FROM the constant — because the consequence clause is absolute.** `TC-B78-31` had done exactly this for the *width* axis all along; nothing had done it for the axis Inc-5 changed. **`C-78-xxvii`: define a floor in the units the OPERATOR receives, not the units the layout reports. `C-78-xxviii`: a boundary constant owes a node on both sides, and the upper side must assert the CONSEQUENCE.**

⚠️ **For Inc-10:** `AT-B78-26`'s *"≥ 1 hex row at 120×30"* must inherit the `>= 2` **painted** form. Read as painted content height it is the same defect, in the increment that closes the batch's headline case.
- **⚠️ Units:** Textual's CSS `width: N` is the **border-box**, so a 22-cell content list is authored `width: 26`. A CSS `width: 22` list is **18 content cells** and moves the breakpoint to **135** (executed: 134 -> 78 fail, 135 -> 79 pass). ⚠️ **M6 — the three regime constants are TERMINAL dimensions, compared against `event.size.width` / `event.size.height`, exactly as the `_apply_width_regime` precedent does (`app.py:6239-6240`). Only the LIST width is in content units** (22 content = CSS `width: 26`). At `W = 139` the content width `C` is **111**, not 139 — an implementer comparing `#diff_columns.size.width` to 139 would not switch regime until **W = 167**. The earlier draft's blanket “content units” sentence reproduced this batch's own **F-5** inside the LLR that corrected it; the name `_DIFF_MIN_H` is likewise renamed `_DIFF_MIN_H` because it is a terminal height, not a row count.
...
- **Validation:** `test (integration)` + `inspection` · **Numeric pass threshold:** the literals `139`, `94` and `29` appear **0** times in the **new** `AT-B78-*` / `TC-B78-*` nodes (C-39). ⚠️ `26` is **dropped as a sentinel** — a two-digit integer any address, size or width can produce, so counting it measures noise (Q-m5/NEW-4). Regime sizes are written `size=(_DIFF_MIN_W, …)` / `(_DIFF_MIN_W - 1, …)`, never as bare integers (m6).
...
- **Statement:** When the terminal fails either deliverability floor, the panel **shall** render a notice naming **every** unsatisfied axis and the value each requires, and **shall not** render an empty results area; and the notice **shall** compose only author-constant text and integers derived from the terminal geometry and the regime constants, and **shall not** interpolate any file-derived string.
- **Geometry:** **80×24 fails BOTH axes** (65 < 79 **and** 24 < 29). `80×30` fails **width only** (2 content rows available); `120×24` fails **height only** (`C = 92` → window 87 ≥ 79). A single "terminal too small" message would be wrong in each single-axis case — and a **singular** "the unsatisfied axis" is wrong in the both-fail case, **which is the only case `AT-B78-29` actually tests** (M7). Hence *every* unsatisfied axis.
- **⚠️ SEC-F2 — the notice is a NEW author surface and is bounded now, at zero cost.** `#diff_status` is `Static(..., markup=False)` (`screens_directionb.py:6759-6764`), i.e. it carries exactly the SEC-F1 gap. "Make the error message more helpful" by naming the loaded image or the two compared paths is the obvious Phase-3 temptation, so the no-file-derived-text clause is **normative**, not advisory.
- **Validation:** `test (integration)` · **Numeric pass threshold:** at **80×24 the notice names BOTH axes** (it fails both); at a width-only failure (`80×30`) it names width alone; at a height-only failure (`120×24`) it names height alone; at `W = _DIFF_MIN_W` / `H = _DIFF_MIN_H` exactly, the notice is **absent**; and the notice's rendered text contains **no** substring of any loaded file path (SEC-F2)
...
- **Geometry:** the executed ladder at 120×30, `#diff_columns` / `#diff_hex_a` **content** rows: shipped **0/0**, compaction alone **3/0**, compaction **and** bar removal **6/2**. (m4: rev-1 and rev-2 recorded the first cell as `1/0`; `#diff_columns` **clips to 0** there, which is what §5.3 and HLR-125's rationale both say. Three lanes measured 0.)
- **Validation:** `test (integration)` · **Numeric pass threshold:** ≥ **1** emitted hex row visible (today 0)
- **Acceptance criteria:** ⚠️ **C-40:** asserted in the **same test** as LLR-125.1 — and revision 2 **makes that true**, where rev-1 asserted it while splitting the pair across two increments. The single node is `AT-B78-26`, landing whole at Inc-10. Separated, the height assertion is invariant under an implementation that compacts the rows and still leaves the result area at zero (executed: compaction alone at 120×30 gives rows 1/1/1, `#diff_status` visible, `#diff_hex_a` content **0**).

### HLR-126 → LLR-126.x

**LLR-126.1 — discoverability without displacing the Footer**
- **Statement:** The run list **shall** carry a visible affordance naming its navigation keys; the set of application bindings with `show` and `enabled` true **shall** equal its pre-change set; and the number of Footer children painted wholly inside the Footer's width at 80×24 **shall** be unchanged.
...
- **Geometry — the model count and the painted count are different quantities, and the painted one is re-derived at revision 3 under §1.3's normative rule.** Executed at three sizes, `Footer.region = Region(x=1, …)` in every one:

  | terminal | `size.width` | children | model (`show and enabled`) | **painted (§1.3 rule)** | frame-mixing form |
  |---|---|---|---|---|---|
  | 80×24 | 78 | 15 | 14 | **8** | 7 |
  | 120×30 | 118 | 15 | 14 | **13** | 12 |
  | 160×40 | 158 | 15 | 14 | **15** | 14 |

  The Footer is **inset by one column**, so its painted span is absolute `[1, 79)`. There is exactly one right-aligned child per size — ending at 79 / 119 / 159, **flush with that edge** — and it *is* painted; the frame-mixing form discards it at every size. **Revision 2's 7 / 12 / 14 was that form.** The conclusion is unchanged and in fact sharper: rev-1's *"all 14, none truncated"* is still wrong (14 ≠ 8), and the honest `origin/main` figure at 80×24 is **8 of 15**. ⚠️ At **160×40 the painted count is 15 of 15**, so *"none truncated"* is **true** there — the false-fail is specific to 80×24, which rev-2's blanket wording did not distinguish. The **set-equality** repair on the model key set is unaffected and correct (executed: `+1 chip → contains-all = True`, `set-equality = False`).
- **⚠️ QA NEW-8 — the painted clause cannot fail where the Footer is saturated, so it is asserted per size.** Executed with the mutation confirmed **applied** (`model 14 → 15`, `children 15 → 16`): painted stays **8** @80×24 and **13** @120×30 — at those widths a new chip lands past the right edge and cannot change the count — and moves **15 → 16** only @160×40. The painted clause is therefore a **regression PIN at 80×24 and 120×30** and a **gate at 160×40**; the set-equality clause on the model key set is what gates the requirement at every size. ⚠️ My first reproduction attempt mutated `S19TuiApp.BINDINGS` in place and the mutation **did not apply** (`model` stayed 14, `children` stayed 15) — Textual resolves `BINDINGS` at class creation. Recorded because *a mutation that does not mutate is a vacuous counterfactual*: the INERT reading it produced was indistinguishable from the real one until the applied-check was added.
- **Validation:** `test (e2e)` + `inspection` · **Numeric pass threshold:** the `show and enabled` key set is **set-equal** to the committed pre-change set of 14; the count of **painted** Footer children under §1.3's rule is unchanged at **8** @80×24, **13** @120×30 and **15** @160×40, asserted as three separate arms; `git diff` over `app.py:1338-1375` → **0** lines (`AT-B78-32`)
...
3. **Every quantified set comes from the RULE, not the implementation** (C-31): the 10 screens from `SCREEN_CONTAINER_IDS`; the owning set computed in the test body from the **stated regex** `(search|goto)_input$` and asserted **set-equal** to `{workspace, a2l, mac}` (HLR-119), with `len(OWNING) == 3` kept only as a redundant guard — the bare length assertion false-fails the obvious tree derivation, which yields **7**; `R` from the fixture's own run list.
...
9. **C-18 node allocation is owed at each increment's gate — one distinct on-disk node per AT id.** This document names **zero** `::` node paths and maps ATs only to files (§7's Files column); that is deliberate for Phase 1, and it is stated here rather than left implicit under §9's "zero orphans" (Q-M6.3).
10. **An invariance AT takes its expectation from a committed pre-change artifact, never from the live producer.** C-31's "derive the set from the rule" applies to *quantified sets over the system*; for an *invariance* claim the rule **is** the pre-change capture. Rev-1 conflated the two and shipped `AT-B78-03` inert (BL-1).
...
**PINs are labelled and are never counted as discharged gates.** Revision 2 corrected the labelling in the direction Phase 2 found it wrong: **three rows carried GATE with no reddening evidence and are now PINs** (`AT-B78-11`, `AT-B78-19`, `AT-B78-22` — the last after its rewrite becomes a real gate). The six original PINs were re-audited by the QA lane and **all six are honest** — none was a quietly downgraded gate.

| AT | Declared subject | Subject in its own expression? | Reddening mutation (substituted VALUE) | Executed transcript | Verdict |
|---|---|:-:|---|---|---|
| `AT-B78-01` | the row is absent on every screen; palette still a child of `CommandBar` | ✅ queries the id, the slot height and the palette's **parent** | none needed | `#command_bar_row present=True height=3`, 10/10 | **GATE — RED today** |
| `AT-B78-02` | Ctrl+K opens everywhere | ✅ | `action_focus_palette` body → `None` | `PRE 10/10 open → MUT 0/10 open → POST 10/10` | **PIN — discharged as a pin** |
| `AT-B78-03` | the palette action set is unchanged | ✅ **after the BL-1 rewrite** — expectation is the Inc-0 artifact, not the producer | drop a `Binding` from `S19TuiApp.BINDINGS`; against the **artifact** the observed set becomes 36 vs an expected 37 | **as-worded-in-rev-1: `PRE (True,37,37) → MUT (True,36,36) → POST (True,37,37)` — INERT.** Against the artifact form the same mutation reddens. ⚠️ Rev-1 also named `action_focus_palette → None` as its mutation; executed, the action set is **invariant** under it (`n` stays 37) — a mutation that cannot touch the subject (Q-m1) | **PIN — rewritten; rev-1's form was INERT** |
...
| `AT-B78-10` | the update PATH moves both surfaces | ✅ `before != after` per surface | remove the successor call from `update_project_labels` | feature does not exist yet | **PIN — discharge owed at Inc-7** |
| `AT-B78-11` | the status bar spends no row | ⚠️ the requirement says *"shall not increase"*; rev-1's expression was the literal `== 7`, **already true** | set the new label to its own row and confirm 7 → 8 | `{all 10 screens: 7}` at 80×24 and 120×30 — a **measurement, not a mutation** | **PIN (was mislabelled GATE) — mutation owed at Inc-7.** Threshold is `1 <= h <= 7`: a bare `== 7` false-fails a conforming *reduction* (Q-m6) and a bare `<= 7` passes a **collapsed** bar (NEW-7), so both bounds are normative and `AT-B78-11` is asserted in the same run as `AT-B78-08` |
| `AT-B78-12` | local search/goto unchanged | ✅ 9-row payload digest | `find_string_in_mem → lambda: None` | `0a159da97fa81714 → 9d6c9b6aeadac6fa → 0a159da97fa81714` (recipe now stated in LLR-121.2) | **PIN — discharged as a pin**; a second mutation on the **go-to** half is owed (Q-m3) |
| `AT-B78-13` | the dead symbols and CSS are gone | ✅ class-qualified AST + selector census | none needed | all **seven** symbols (1 definition each, executed) + all six selectors exist | **GATE — RED today** |
| `AT-B78-14` | the registry guard | ✅ | none needed | 4 LIVE rows name the doomed nodes | **GATE — RED after deletion** |
| `AT-B78-15` | every run reachable **by keyboard**, set-equal to `range(R)` | ✅ | none needed | `type=Static can_focus=False` | **GATE — RED today** |
| `AT-B78-16` | every run reachable **by mouse**, incl. one past the viewport | ✅ distinct observable (click + `scroll_visible`) | none needed | `ListView` descendants under `#diff_columns` = **0** | **GATE — RED today** |
| `AT-B78-17` | exactly one row **visibly** selected, ≥2-row guard | ✅ resolved `(background, color, text_style)` triple | none needed | no row exists to carry a selection style | **GATE — RED today** |
| `AT-B78-18` | the caps and their notice survive | ✅ **after the F-6 rewrite** | (a) `DISPLAY_MAX_RUNS` **128 → 100000** · (b) `_render_run_list(total_runs=…)` → `len(self._runs)` | `GREEN → RED (rendered_rows=200, notice=None) → GREEN` and `GREEN → RED (notice=None) → GREEN`. ⚠️ With the guard placed **before** the capture the first mutation raises in the guard rather than reddening the assertion; LLR-122.3 now places it **after**, so both transcripts are of the specified predicate (Q-m2) | **PIN — discharged, after rev-1's first form proved INERT** |
| `AT-B78-19` | App keys survive **off-list** focus | ⚠️ rev-1 had **no precondition assertion**, so the subject was never reached | add `Binding("k", …)` to the run list's `BINDINGS` and confirm the Legend no longer opens | `run list type=Static can_focus=False` · `set_focus(#diff_range_list)` lands on **`RailItem(id='rail_item_workspace')`** · `k` opens the Legend anyway — **green with its precondition silently false** | **PIN (was mislabelled GATE) — mutation owed at Inc-2.** The restored LLR-122.2 blur-discipline criterion adds `assert app.focused is <the run list>` as a precondition |
...
| `AT-B78-22` | the span covers the run ± context | ✅ **only after the Q-M1 rewrite** — expectation is now three literal addresses | `DISPLAY_CONTEXT_BYTES` **16 → 64** | **as-worded-in-rev-1: `PRE ctx=16 (True, want 3, emitted 5) → MUT ctx=64 (True, want 9, emitted 11) → POST (True,3,5)` — INERT under a 4× change to its own subject.** The literal form reddens | **rev-1 was a mislabelled PIN; the rewritten form is a GATE, discharged at Inc-4** |
| `AT-B78-23` | no hex row wraps, **wide regime** | ✅ widest emitted vs content width | none needed | `79 > 42` @160×40, `79 > 30` @132×44 | **GATE — RED today** |
| `AT-B78-24` | no hex row wraps, **fallback regime** | ✅ same, plus clipped **height ≥ 1** (M3) | none needed | `79 > 26` @120×30 | **GATE — RED today**; lands at Inc-10 (BL-2) |
| `AT-B78-25` | the regimes are observably different | ✅ lands whole, not by arm (Q-M6.2) | force one regime for all widths and confirm the wide/fallback layout properties become identical | *owed at Inc-10* | **GATE — discharge owed** |
| `AT-B78-26` | rows clip to 1, status visible, **and** ≥1 hex row at 120×30 with the bar removed | ✅ the joint clause is what makes it a gate | none needed | `120×30: rows 3/3/3, #diff_columns clipped=(92,0), #diff_status clipped=(92,0)` | **GATE — RED today** (absorbs the retired `AT-B78-27`) |
| `AT-B78-28` | discoverability without displacing the Footer | ✅ **set-equality on the model key set is the gate**; the painted-count clause is a **per-size PIN below 160×40** (QA NEW-8) | add `Binding("ctrl+alt+z", …, show=True)` — **applied**, `model 14 → 15`, `children 15 → 16` | **model arm:** containment form → True (**NOT reddened**), set-equality form → False (**reddens**). **painted arm, per size:** `80×24 8 → 8` **INERT** · `120×30 13 → 13` **INERT** · `160×40 15 → 16` **REDDENS** | **GATE on the model arm; the painted arm is a PIN at 80×24 / 120×30 and a GATE only at 160×40** |
| `AT-B78-29` | the notice names its unsatisfied axis/axes | ✅ | force the notice text to a single generic string | 80×24 renders nothing today | **GATE — RED today**; moved to Inc-5 (width-only) |
| `AT-B78-30` | both display forms, both surfaces | ✅ four observations | flatten to one form | `(1/1)` is **not** produced today (P-38c) | **GATE** |
| `AT-B78-31` | the persisted report is complete | ✅ counts entries in the **written file** | route the report off `panel._runs` | correct today by construction | **PIN — mutation owed at Inc-3** |
| `AT-B78-32` | 🆕 the App `BINDINGS` block is untouched | ✅ zero-line `git diff` over `app.py:1338-1375` | add any binding to the block | *restored architect observable (§5.6.2); owed at Inc-6* | **PIN — mutation owed at Inc-6** |
| `AT-B78-33` | 🆕 compaction actually enlarges the result area | ✅ `#diff_hex_a.size.height` at 132×44 vs the **Inc-0 artifact's** pre-change baseline, never an inline literal (rule 10) | revert the row-height rule and confirm 13 → 7 | **executed: shipped `(7,11)` → compacted `(13,17)`** — a strict increase with no Lane-1 work | **GATE — RED today** (BL-3's replacement for Inc-1's invariant gate) |

**Restore proof.** Every mutation was applied **in-process** (monkeypatch / class-attribute rebinding), reverted in the same process, and the predicate re-run. `git diff --stat -- s19_app/ tests/` is **empty**. A hash alone would not prove this — a same-size in-place edit plus a sub-second restore is invisible to `git status` — so the proof is the predicate returning GREEN plus an empty source diff.

**Count.** **24 GATEs · 9 PINs.** Rev-1 reported 6 PINs; the three additions (`AT-B78-11`, `AT-B78-19`, and rev-1's form of `AT-B78-03`) are relabels **toward** honesty, and `AT-B78-22` moves the other way only because it was rewritten into a real gate.
...
| US-78-3 | project + A2L readable everywhere, both forms, control-char safe | `#workspace_status_bar` · `#loaded_panel` | `AT-B78-08…11`, `AT-B78-30` | ✅ project absent 10/10 |
...
| US-78-7 | no wrapped row where deliverable; a notice naming every failed axis where not | `#diff_hex_a.size.width` · panel text | `AT-B78-23…25`, `AT-B78-29` | ✅ `42 < 79`, `30 < 79`; 80×24 silently empty |
| US-78-8 | a comparison at 120×30 shows bytes | clipped + content heights | `AT-B78-26` (absorbing 27), `AT-B78-33` | ✅ clipped height 0; 132×44 content 7 |
| US-78-9 | the keyboard path is findable, Footer undisplaced | help panel · `active_bindings` · painted Footer children | `AT-B78-28`, `AT-B78-32` | ✅ list not focusable |

**Functional chain (white-box):** HLR-118 → LLR-118.1…3 → `TC-B78-01…03, 40` · HLR-119 → LLR-119.1…4 → `TC-B78-04…08, 41, 46` · HLR-120 → LLR-120.1…5 → `TC-B78-09…13, 42, 43` · HLR-121 → LLR-121.1…4 → `TC-B78-14…16, 44` · HLR-122 → LLR-122.1…4 → `TC-B78-17…22, 47, 48` · HLR-123 → LLR-123.1…3 → `TC-B78-23…28, 45` · HLR-124 → LLR-124.1…4 → `TC-B78-29…33` · HLR-125 → LLR-125.1…2 → `TC-B78-34…37` · HLR-126 → LLR-126.1 → `TC-B78-38, 39`.
...
**Rule applied (corrected at revision 3):** the architect allocation supplies the **id namespace**; **neither lane's content is a superset of the other's** — §5.6 exists because that premise was false and cost two lost acceptance criteria. Every QA AT is mapped **by subject, never by number**. **Zero QA ATs are unaccounted for**, re-verified by the Phase-2 QA lane.
...
| `AT-B78-21` | clipped heights, rows 1, results ≥3 | **`AT-B78-26`** (one node) | **MERGED** | Same subject. **QA's `#diff_status` visible clause adopted** — rev-1's form omitted the status line, which also clips to 0 today. ⚠️ Rev-1 split this across `AT-B78-26`/`-27` in two increments while `LLR-125.2` required one test; revision 2 **retires `-27` into `-26`** (§5.6.2, Q-M6.1). |
...
**QA `TC` reconciliation:** `TC-01` (slot height) → merged into `AT-B78-01` + `TC-B78-40` · `TC-02` (routing on `_active_screen_key`) → 🆕 `TC-B78-41` · `TC-03` (C-17 both new sinks) → merged into `TC-B78-12` · `TC-04` (`DISPLAY_CONTEXT_BYTES` retires/floors) → 🆕 `TC-B78-45` · `TC-05` (80×24 bounded degradation) → merged into **`TC-B78-34`** and `AT-B78-29`, **upgraded from a pin to a gate by R-1** (m7: rev-1's table pointed this at `TC-B78-32`, which is the *120×24 height-only* case) · `TC-06` (pre-change artifact capture) → 🆕 `TC-B78-44` · `TC-07` (palette entry set) → merged into `AT-B78-03`, **decided at 37** (D-3) · `TC-08` (per-arm CC-1 harness) → folded into §5.1 rule 7 as a standing gate rule rather than a node. **3 merged · 3 new · 2 folded.**

### 5.6 REVERSE audit — architect lane → canonical (added at revision 2)

**Why this section exists.** Revision 1's §5.5 stated *"the architect allocation is canonical (**superset**)"* and audited **QA → canonical only**. **That premise is false and the one-directional audit hid it.** A merge can lose the content of the lane it treats as the base, because the merging author is the one at risk of quietly preferring — and then quietly dropping — their own rows. The Phase-2 architect lane found the proof; this table is the guard that should have existed in revision 1.

**Corpus.** All **19** `- **Acceptance criteria:**` lines and all **30** AT / **39** TC subjects in `01-requirements-architect.md`, checked by grep against the merged document.

#### 5.6.1 The 19 architect LLR acceptance criteria

| # | Architect acceptance criterion | Disposition | Reason |
|---|---|---|---|
| LLR-118.1 | *"`#command_palette` remains a direct child of `CommandBar` … the palette must not move screens"* | 🔴 **WAS LOST → RESTORED** | Merged LLR-118.1 had **no acceptance-criteria line at all**, and the surviving co-assertion `len(query("#command_palette")) == 1` is satisfied **wherever the palette lives** — while HLR-118's entire premise is that the palette is a sibling the row deletion must not disturb. Restored as an HLR-118 **Statement** clause and as a parent-identity co-assertion in `AT-B78-01`. |
| LLR-118.2 | `palette_is_open` is a **property**, not a method | ✅ **MERGED** | present in LLR-118.2 |
| LLR-119.1 | the map's key set is **set-equal to `SCREEN_CONTAINER_IDS.keys()`** | ✅ **MERGED** | present, and strengthened at revision 2 by the `OWNING` derivation pattern (Q-M3) |
| LLR-119.2 | *"the gate arm is the **status-text delta**"* | 🟡 **SUPERSEDED — reason recorded** | `set_status` writes the **log tail**, not `#status_text` (P-40b, executed). The status-text form would have been vacuous. Replaced by the `#log_line_4` + line-count delta. |
| LLR-119.3 | `pilot.press("escape")` only; `.blur()` does not discharge | ✅ **MERGED** | present in LLR-119.3 |
| LLR-120.1 | two update paths ⇒ two arms | ✅ **MERGED** | present in LLR-120.1 and in `AT-B78-10` |
| LLR-120.2 | remount with **classes only**; no `_nodes`/`_context` member | ✅ **MERGED** | present in LLR-120.2 |
| LLR-120.5 | both forms on **both** surfaces = **four observations** | ✅ **MERGED** | present in LLR-120.5's threshold |
| LLR-121.1 | the census is an **AST** census, not a line regex (C-42 mechanic 5) | ✅ **MERGED** | present; **strengthened** at revision 2 to *class-qualified* and to **seven** symbols (M4 / Q-m4) |
| LLR-121.2 | the control is captured from the **pre-change** tree in its own commit | ✅ **MERGED** | present in LLR-121.2 and Inc-0 |
| LLR-121.3 | `_meta.next_free` / `high_water` **not** touched | ✅ **MERGED** | present in LLR-121.3 |
| LLR-122.2 | *"`pilot.press` only, with the LLR-119.4 blur guard"* | 🔴 **WAS LOST → RESTORED** | Merged LLR-122.2 had **no acceptance-criteria line**. QA's Q-M5 found the consequence independently: `AT-B78-19` is **green today with its precondition silently false** — `set_focus` on the `Static` run list does not take and focus lands on `RailItem(id='rail_item_workspace')`, yet `k` opens the Legend anyway. Restored **with the mirror-image precondition assertion** the loss had removed. |
| LLR-122.3 | *"the AT quotes `DISPLAY_MAX_RUNS`, never the literal `128`"* | 🟡 **SUPERSEDED — reason recorded** | This is the **F-6 shape**: an expectation read from the class under test. Deliberately replaced by the fixture-independent form. The *guard* still quotes the constant; the *expectation* must not. |
| LLR-123.2 | *"the floor is a floor, not the value"* | ✅ **MERGED** | present in LLR-123.2 |
| LLR-123.3 | `#diff_hex_a/b` are `markup=False` and **shall remain so** | ✅ **MERGED** | present in LLR-123.3 |
| LLR-124.1 | *"do not overload `width-narrow`"* (11 dependent rules) | ✅ **MERGED** | present in LLR-124.1 |
| LLR-124.4 | *"explicitly a **regression pin**, not a gate"* | 🟡 **SUPERSEDED — reason recorded** | Operator ruling **R-1** turned the silently-empty panel into a defect to fix, so the notice became a **gate** (`AT-B78-29`). A supersession that *strengthens* the node, recorded rather than silent. |
| LLR-125.1 | Compare/Report handlers unchanged; check for node duplication first | ✅ **MERGED** | present; and revision 2 acts on the duplication warning by naming `test_tc029` (Q-M2) |
| LLR-125.2 | the pair is asserted in the **same test** | ✅ **MERGED**, and revision 2 **makes it true** | Revision 1 carried the clause while splitting the pair across Inc-1 and Inc-9 (Q-M6.1). `AT-B78-26` now absorbs `AT-B78-27` into one node at Inc-10. |

**Outcome: 14 MERGED · 3 SUPERSEDED-with-reason · 2 WAS-LOST-now-RESTORED · 0 silently dropped.**

#### 5.6.2 Architect AT/TC subjects overwritten at the same id

| Architect id + subject | What happened | Disposition |
|---|---|---|
| `AT-B78-29` = *"the no-regression arm — `git diff` over the App `BINDINGS` block is 0 lines"* | The **id** was reused for the 80×24 notice; the observable survived only as a threshold clause with **no node** | 🆕 **RESTORED as `AT-B78-32`**, realized at Inc-6 |
| `TC-B78-18` = *"exactly one run — `up`/`down` are **no-ops, no crash**"* and `TC-B78-19` = *"`up` on the first entry, `down` on the last"* | Both ids were reused (cap boundary; 1-run style guard). The **list-edge no-op** case ended with no node anywhere, and merged HLR-122's catalog carried **no ☑ error class** | 🆕 **RESTORED as `TC-B78-47`** (list-edge no-op, error class) |
| `TC-B78-03/07/10/22/33/37` subjects (reported by the architect lane as delegated-and-unverified) | Re-checked at the fold against the merged catalogs | ✅ **all six subjects present** under merged ids — the architect lane correctly flagged these as unverified, and they reconcile |

#### 5.6.3 What the reverse audit changes about §5.5's claims

- `§5.5`'s *"the architect allocation is canonical (superset)"* is **withdrawn**. The correct rule: **the architect allocation is the id namespace; neither lane's content is a superset of the other's.**
- `§5.5`'s *"0 RETIRED · No QA observable was dropped"* remains true **for the QA direction** — independently re-verified by the Phase-2 QA lane (*"§5.5 accounts for 22 … No QA observable was dropped"*).
...
>
> **C-21 discharge.** `TC-B78-49` is owned by **Inc-2**, which realized it in the same increment that introduced the defect. No other increment's AT/TC allocation moves.
>
> ⚠️ **This discharge was FALSE when first written (re-gate F8), and the correction is recorded rather than overwritten.** The original text asserted that *"§7's file map is unchanged apart from the `app.py` cell"* — but **§7 had not been edited at all.** The whole amendment was one hunk, here in §5.7, so the discharge asserted a consistency that did not exist: §7 still listed `styles.tcss` for Inc-2 (never touched, sha unchanged), still mapped `app.py | 5, 7, 9, 11` with Inc-2 absent, and named neither `TC-B78-49` nor the prototype file. **§7's four cells are now actually edited.** The defect is exactly the one §6.4a exists to catch — a change applied where it was noticed and not swept to its dependents — landing inside the discharge of the control written to prevent it, and committed by the orchestrator. `enum.py` did not catch it because its tracked terms do not include §7's file-map cells; **that gap is `C-78-xi`.**
...
| **F-4** | **A phantom value in an acceptance.** `TC-B78-11` first asserted `(1/1)` for a single-variant project. `app.py:11333` gates the counter on `> 1` and `tests/test_tui_variants.py:259` pins the plain form — a **C-36 phantom that would have false-failed a correct implementation while contradicting a shipped acceptance.** Root cause: *a display form asserted without executing it.* |
| **F-5** | **A breakpoint reported without its units.** **135** (18-content-cell list, authored CSS `width: 22`) vs **139** (22-content-cell list) are the same bound at two list widths; my *reporting* was wrong because a breakpoint with no unit is not re-derivable. ⚠️ **Attribution corrected at revision 4:** the architect lane reported the *internal contradiction* and named **135** as correct — it never asserted 136. My revision-2 note framed this as a rejection of the reviewer's figure; it was not, and the framing was mine. Reconciled across 9 widths (P-32b/P-34b) and re-swept 130-140 at the fold, which corrected my own coarse 136 to **135**. |
...
| **F-9** | 🆕 **Revision 1 shipped the F-6 shape it had just caught and generalised — twice.** `AT-B78-03` derived its expected palette set from `app._build_palette_entries()` while the widget under test *is constructed from that same call* (`f(x) == f(x)`, executed inert at 36 == 36), and `AT-B78-22` read its expected span from `DISPLAY_CONTEXT_BYTES` (executed inert under 16 → 64). Both were caught by reviewers, not by me. **The lesson is not that the control failed — it is that a lesson recorded in prose is not a control.** The two rev-1 lessons that were violated in a *mechanically checkable* form (the modal-keyword scan, the id-completeness scan) both held; the ones violated were stated only as prose. |
| **F-10** | 🆕 **A reconciliation audited in one direction is not audited.** §5.5 reconciled QA → canonical and asserted "0 RETIRED" under a *superset* premise that was false. Two architect acceptance criteria — LLR-118.1's palette-position invariant and LLR-122.2's blur discipline — existed in neither the merged document nor a retirement line, and **both losses had consequences a reviewer then found independently** (`AT-B78-01`'s co-assertion satisfied wherever the palette lives; `AT-B78-19` green with its precondition silently false). §5.6 is the guard that should have existed in revision 1. |
| **F-11** | 🆕 **My verification probe for Q-M4 crashed on the binding-tuple shape** (`'ActiveBinding' object has no attribute 'key'`) — the identical failure the QA lane recorded as its own F-7, in the same session, on the same API. No conclusion was drawn from the crashed run; the corrected reader produced the 7-of-14 figure. Recorded because two lanes hitting the same undocumented shape is a signal about the API, not about either lane. |
...
### 6.4a Propagation register — **a GATE, generated by `enum.py`** (revision 4)

**Three revisions, three stages of the same control.** Revision 2's register failed at the **input set** — hand-recalled sites, so a site I had not thought of could not appear. Revision 3 derived the input set but dispositioned it **by hand**, so its counts stopped reproducing against the file it shipped with (QA NEW-10: 2 of 14 regexes matched exactly; the Footer row promised *"disposition of every hit"*, claimed 6, dispositioned 4). The architect lane named the missing stage exactly:

> *A derived enumeration has three stages. Revision 2's defect was at the input set; revision 3's is at the disposition. The register is now derived; the accounting of it is still by hand.*

**Revision 4 closes it: the register is a script that gates.** `.dev-flow/2026-08-06-batch-78/enum.py` derives the input set **and** the disposition, prints the table below from its own output, and **exits non-zero** on any surviving stale form or any hit outside its term's allowed sections. It is re-run, never re-read; the table cannot drift from the file because it is generated from it.

**Two checks, both mechanical:**
1. **Stale-form gate** — 15 superseded spellings that must have **zero** matches. A match means a fix was reverted, or a new site was written in the old form.
2. **Disposition gate** — every hit of every tracked term must sit in a section that term is allowed to appear in. Anything else prints `UNDISPOSITIONED` with its line and text, and fails the run.

**Executed against the final file — the passing transcript, not a prediction:**

```
$ python .dev-flow/2026-08-06-batch-78/enum.py
§6.4a propagation gate — 01-requirements.md, 973 lines
STALE FORMS (each must be 0)     ... all 15 -> 0  ok
TRACKED TERMS                    ... 20 terms, 0 undispositioned
GATE: PASS — 0 stale forms, 0 undispositioned hits            (exit 0)
```

**On its first run against revision 3 it FAILED with exit 1**, naming `:656` — `AT-B78-28`'s row still carrying the *"7, not 14"* baseline (architect NEW-8) — and the superset premise. **Both were findings the Phase-2 re-gate had to report by hand. The gate now finds them itself**, which is the entire difference between a register that reports and one that gates.

| Changed term | grep pattern | Hits | Sections carrying them |
|---|---|:-:|---|
| **OWNING length guard** | `len\(OWNING\) == 3` | **3** | §3 · §5.1 · §5.5 |
| **status-bar height** | `status_bar\.size\.height|<= 7\b` | **6** | §3 · §4 · §5.3 · §6.4a |
| **unsatisfied axis** | `unsatisfied axis` | **8** | §2.8 · §3 · §4 · §5.3 · §6.4a |
| **clipped vs content bound** | `≤\*?\*? its (clipped|\*\*content\*\*)` | **2** | §3 · §4 |
| **footer painted count** | `painted|none truncated|contains all` | **22** | §1.3 · §2.4 · §3 · §4 · §5.3 · §5.4 · §6.4a |
| **footer numeral** | `\b(7|8|12|13|14|15|16)\b(?=[^|]{0,60}(chip|Footer|painted))` | **11** | §2.4 · §2.7 · §2.8 · §3 · §4 · §5.5 · §6.4a · §9 |
| **fallback layout** | `fallback` | **13** | §2.8 · §3 · §4 · §5.3 · §6.4a · §7 · §8 |
| **symbol count** | `\b(six|seven)\b[^|]{0,20}symbols?` | **4** | §3 · §5.3 · §5.6.1 · §7 |
| **_project_label sites** | `_project_label` | **13** | §2.7 · §3 · §4 · §5.3 · §6.5 · §6.4a · §7 |
| **_KIND_LABEL** | `_KIND_LABEL` | **3** | §4 · §6.4a · §8 |
| **superset premise** | `superset` | **8** | §3 · §5.5 · §5.6 · §5.6.3 · §6.3 · §6.4a · §10.3 |
| **26 sentinel** | ``26`` | **3** | §3 · §4 · §6.4a |
| **regime constants** | `_DIFF_MIN_H|_DIFF_WIDE_MIN|_DIFF_MIN_W` | **15** | §1.3 · §2.8 · §3 · §4 · §6.4a · §8 |
| **markup=False** | `markup=False` | **11** | §2.4 · §4 · §5.6.1 · §6.4a |
| **AT-B78-27 retirement** | `AT-B78-27` | **10** | §3 · §5.3 · §5.6.1 · §5.6.3 · §5.7 · §6.4a · §7 · §10.1 |
| **135 / 136** | `\b13[56]\b` | **6** | §0 · §2.7 · §4 · §6.3 · §6.4a · §10.2 |
| **test_tc029** | `test_tc029` | **5** | §4 · §5.6.1 · §6.4a · §7 |
| **AT-B78-22 mutation** | `DISPLAY_CONTEXT_BYTES == 16|high = end` | **3** | §3 · §6.4a · §7 |
| **AT-B78-33 baseline** | `AT-B78-33` | **9** | §3 · §5.3 · §5.4 · §5.7 · §6.4a · §7 |
| **shall outside Statements** | `\bshall\b` | **45** | §0 · §3 · §4 · §5.3 · §5.6.1 · §6.4a · §9 |

**Scope note.** The allowed-section lists are themselves part of the gate: widening one is an explicit, reviewable act rather than a silent pass. Three widenings in this pass were legitimate sites the census surfaced that revision 3's hand-written table had never listed — which is the point.

**Adopted as Phase-3 gate checks** (the architect lane's two cheap mechanical assertions): **(a)** every `LLR-nnn.n` appears in some increment's Content cell — the check that would have caught BLK-A; **(b)** every figure appearing more than once appears with one value.
...
**Why this is an amendment and not a test relaxation** — the distinction matters, because relaxing a threshold to match an implementation is how a gate quietly stops gating. The literal three addresses are still asserted **exactly**, at the one pane height where exactness is satisfiable (132×24, content height 0 — the size `HLR-123`'s own rationale was measured at). At 132×44 the same three are asserted **contained**, and that containment arm is **declared inert under M2** rather than counted as a discharge: `high = end + ctx → high = end` reddens the exact arm, not the containment one. The unsatisfiable half was split into two halves that are each satisfiable, and the weaker one is labelled.

⚠️ **Correction (Inc-4 gate review, F-4): my original wording here said "nothing was weakened", and that is an OVERCLAIM.** Something was traded. An **exact twelve-literal** set at 132×44 *would* have reddened under M2, which shifts the window start `0xFB0 → 0xFA0`; the containment form does not. What containment buys is stability under geometry change — a twelve-literal threshold is re-baselined by any future increment that moves the pane by one row, which is the `C-29` failure mode this batch has already paid for twice. **The trade is defensible and is recorded here as a decision, not as an absence of loss.** The exactness that survives is asserted at content height 0, where M2 still reddens it.
...
- **Verifier impact:** `tc038` is re-pointed at the two new surfaces; the display-form contract (LLR-005.3/005.5) moves with it, guarded by the **14** re-pointed `_project_label()` call sites (LLR-121.4).

---

## 7. Increment plan (dependency-ordered) — **Lane 2 first, Lane 1 last (R-4)**

**Why this order.** The conflict between D-10 (S-8 first) and C-30 (snapshot sequencing) dissolves on measurement: at **132×44** the diff result area already has **7 content rows today**, so Lane 2's acceptances are observable **without any Lane-1 work**. C-30 therefore governs — S-1 drifts **29 of 29** cells while each Lane-2 increment drifts **1**, so Lane 1 first would `xfail` 28 cells from increment 1 and ship all of Lane 2 with no snapshot coverage.

**BL-2 — the dependency I mis-scheduled in revision 1.** Revision 1 put `AT-B78-27`, `AT-B78-24`'s 120×30 arm and `AT-B78-25`'s fallback arm at **Inc-9**, stating they need US-78-1 — which is `HLR-118`, at **Inc-10**. Re-executed at the fold, `#diff_hex_a` as `(content, clipped)`:

```
120x30   shipped (0, 0)   S-8 only (0, 4)   S-8 + S-1 (2, 6)
132x44   shipped (7,11)   S-8 only (13,17)  S-8 + S-1 (16,20)
160x40   shipped (3, 7)   S-8 only (9,13)   S-8 + S-1 (12,16)
 80x24   shipped (0, 0)   S-8 only (0, 0)   S-8 + S-1 (0, 1)
```

At Inc-9 the 120×30 **content** height is **0**, so those arms cannot go green: **a correct implementation would fail Inc-9's gate.** The historical resolution of an unsatisfiable gate in this project is a weakened acceptance, so the arms are **moved, not softened** — every one keeps its full predicate and lands where its prerequisite exists. `AT-B78-29` (80×24) is **width-only** — `C = 70` → window 65 < 79 fails regardless of the bar — so it moves **earlier**, to Inc-5, where the notice regime is built.

**BL-3 — Inc-1 had an invariant gate.** `AT-B78-26`'s row-height clause is green with the result area still at zero, which is precisely the state `LLR-125.2` exists to prevent. Inc-1 now gates on a real observable that needs no Lane-1 work: **at 132×44 compaction alone moves `#diff_hex_a` content from 7 to 13** (executed above). That is the new `AT-B78-33`.

**Q-M6 / C-18 — one AT, one node.** `AT-B78-26` and `AT-B78-27` were required by `LLR-125.2` to be *one test* while being assigned to different increments. `AT-B78-26` now **absorbs** the joint clause and lands whole at Inc-10; **`AT-B78-27` is retired**, its observable preserved inside `AT-B78-26` (§5.6). `AT-B78-25` is likewise no longer split by arm — it lands **whole** at Inc-10, and Inc-5 keeps the narrower `AT-B78-23` for the wide layout.

| Inc | Content | ATs realized | Files | Gate |
|---|---|---|---|---|
| **Inc-0** | Pre-change artifact capture: the 9-row search/goto payload, the 37-action palette list **and `#diff_hex_a.size.height` at 132×44**, **own commit, before any production edit** | (enables `AT-B78-03`, `AT-B78-12`, `AT-B78-33`) | `tests/test_tui_commandbar.py`, `tests/_artifacts/…`, `.gitattributes` (3) | artifacts committed; captured from the **shipped surface** (C-35); `git show` confirms the stored blob. **BL-1: this artifact is `AT-B78-03`'s only oracle** |
| **Inc-1** | **HLR-125** — compact the control rows *(Lane 2 opens)* | `AT-B78-33` | `styles.tcss`, `tests/test_tui_diff_screen.py` (2) | full `test_tui_directionb.py` (C-34); **`AT-B78-33`: `#diff_hex_a.size.height` at 132×44 strictly greater than the pre-change baseline read from the Inc-0 artifact** (executed: 7 → **13**). ⚠️ The baseline is **not** an inline literal — §5.1 rule 10 applies, and Inc-0 already writes a pre-change artifact, so the 132×44 height is added to it (NEW-5), asserted in the same run as the three 1-row clauses and the `#diff_status` visibility clause |
| **Inc-2** | **HLR-122** — selectable run list + the C-38 sweep + **the `test_tc029` rewrite (Q-M2)** | `AT-B78-15…19`, **`TC-B78-49`** | `screens_directionb.py`, **`app.py`**, `tests/test_tui_diff_screen.py`, `tests/test_tui_diff_compare_realpath.py`, **`prototypes/cmdbar_a2bdiff.tui_prototype.py`** (5) | C-34 + full `test_tui_diff_compare_realpath.py` + full `test_tui_patch_big.py` (C-26, 25 refs / 5 files); `TC-011` green; **`test_tc029` reddens under `cap 128 → 100000` after the rewrite`**; **`TC-B78-49` reddens alone when route B's awaits are removed** |
...
| **Inc-4** | **HLR-123** — windows follow selection; height-derived rows | `AT-B78-20…22` | `screens_directionb.py`, `tests/test_tui_diff_screen.py` (2) | C-34; `AT-B78-21`'s strict inequality green at **132×44 vs 132×60**; **`AT-B78-22` reddens on its ASSERTION under the implementation mutation `high = end + ctx → high = end`** (the constant mutation raises in the guard and never reaches the assertion — NEW-3) |
| **Inc-5** | **HLR-124 in full — `LLR-124.1` (constants + regime class), `LLR-124.2` (wide layout), `LLR-124.3` (fallback: overlay + paginated viewport), `LLR-124.4` (notice regime)** | `AT-B78-23`, `AT-B78-29` | `screens_directionb.py`, `app.py`, `styles.tcss`, `tests/test_tui_diff_screen.py` (4) | C-34; C-22 per-cell census on the single diff cell; **`AT-B78-29` is width-only and passable here**. ⚠️ **`LLR-124.3` is BUILT here and OBSERVED at Inc-10** — its 120×30 arms need S-8 *and* S-1, which do not exist yet; that split is stated, not implied |
| **Inc-6** | **HLR-126** — discoverability | `AT-B78-28`, `AT-B78-32` | `screens_directionb.py`, `tests/test_tui_diff_screen.py` (2) | C-34; `AT-B78-32`'s zero-line `git diff` over `app.py:1338-1375` |
| **Inc-7** | **HLR-120** — context onto both surfaces, both display forms, **`safe_text` on both** *(Lane 1 opens)* | `AT-B78-08…11`, `AT-B78-30` | `app.py`, `screens_directionb.py`, `styles.tcss`, `tests/test_tui_commandbar.py` (4) | C-34; **`TC-B78-12`'s control-character limb green on both surfaces**. ⚠️ `tests/test_tui_app.py`'s three stubs are **not** evidence (P-38b) |
| **Inc-8** | **LLR-121.4** — re-point the 14 `_project_label()` call sites + the `test_tui_directionb.py` reads, **while both surfaces exist** | (protects `AT-B78-30`) | `tests/test_tui_patch_variant.py`, `tests/test_tui_variants.py`, `tests/test_tui_directionb.py` (3) | those three suites green; `grep -rn 'cmdbar_project' tests/` → 0 |
| **Inc-9** | **HLR-119** — re-home `/`·`g`; the notice at the log tail; the `Esc` release | `AT-B78-04…07` | `app.py`, `tests/test_tui_commandbar.py` (2) | C-34; `_PRE_BATCH_BINDINGS` unchanged; **`OWNING` asserted set-equal, not by length** |
| **Inc-10** | **HLR-118** — delete `#command_bar_row` + CSS `:66-102`. **Plus the four arms BL-2 moved here**, which need S-8 **and** the bar's rows. **No production code for HLR-124 is written here — Inc-10 OBSERVES the fallback layout built at Inc-5**, using the test file it already owns | `AT-B78-01…03`, `AT-B78-24`, `AT-B78-25`, `AT-B78-26` | `command_bar.py`, `styles.tcss`, `tests/test_tui_commandbar.py`, `tests/test_tui_diff_screen.py` (4) | C-34; palette **10/10**, **37** entries **from the Inc-0 artifact**, palette still a **direct child of `CommandBar`**; per-arm verdicts at 80×24 / 120×30 / 132×44 / 160×40 |
| **Inc-11** | **HLR-121** — delete the messages, adapters and helpers; reconcile the registry | `AT-B78-12…14` | `command_bar.py`, `app.py`, `tests/test_tui_commandbar.py`, `tests/test_tui_directionb.py`, `AT-TC-REGISTRY.jsonl` (5) | C-34 **and** `test_id_registry.py` G1–G7; **seven-symbol, class-qualified** AST census; `git diff` over the six handlers = 0 |
| **Inc-12** | **Snapshot regen — canonical CI only**, one PR | — | 29 snapshot SVGs | full suite (**FULL** form); C-22 per-cell |

**BLK-A — `LLR-124.3` had no implementing increment, and this is BL-2's own failure mode one level down.** Revision 1's Inc-5 read *"HLR-124 wide **+ fallback** layouts"*; revision 2 dropped the word when the observing ATs moved out, so the fallback overlay and its paginated viewport had **no increment that builds them** — while `AT-B78-25` (*"the viewport paginates"*) landed at an Inc-10 whose file set holds neither `screens_directionb.py` nor `app.py`. Found independently by both reviewer lanes.

**Two valid fixes existed and I took the one Inc-5's file set already supports.** The coordinator and the QA lane proposed adding `screens_directionb.py` to Inc-10 (→ 5 files, within the cap); the architect lane proposed restoring the build to Inc-5. **I took the architect lane's — it proposed restoring the build to Inc-5, and that is what shipped** (the Inc-10 variant was the coordinator's and the QA lane's, and the QA lane has since formally withdrawn it). Two reasons an enumeration of §7's own map makes concrete: (i) Inc-5 already owns **both** production files the fallback needs — `screens_directionb.py` for the overlay widget and `app.py` for the regime toggle — whereas the additive fix would have supplied only the first; (ii) it would have put diff-panel layout work inside the command-bar-deletion increment, mixing lanes in exactly the increment §7 declares strictly sequential to avoid. Inc-10 stays at 4 files and observes.

**File × increment map (M8 — revision 1's *"no increment edits a file another in-flight increment edits"* was false by enumeration).**

| File | Increments |
|---|---|
| `styles.tcss` | 1, 5, 7, 10 — ~~2~~ **measured unnecessary at Inc-2**: the id rule does not outrank `ListView`'s `DEFAULT_CSS`, because `overflow_y` stays `auto` and the highlight rules live on `ListItem`, an element the id selector cannot reach |
| `screens_directionb.py` | 2, 4, 5, 6, 7 |
| `app.py` | **2**, 5, 7, 9, 11 — **Inc-2 added by operator ratification** (route B: `async def on_ab_diff_panel_compare_requested`, two lines). The `ListView` swap made the rebuild path asynchronous, and no in-plan file could carry the awaited handler |
| `prototypes/cmdbar_a2bdiff.tui_prototype.py` | **2** — **tracked**, and it calls `render_comparison`; once that became a coroutine it would have silently stopped executing it |
| `tests/test_tui_diff_screen.py` | 1, 2, 3, 4, 5, 6, 10 |
| `tests/test_tui_commandbar.py` | 0, 7, 9, 10, 11 |
| `tests/test_tui_directionb.py` | 8, 11 |

**Three production files cross the lane boundary, not one.** The plan is therefore explicitly **strictly sequential**: no two increments are ever in flight and none may be parallelised. This matters concretely — R-9 records a parallel session writing in this tree, and revision 1's wording invited an executing session to run the lanes concurrently.

**Inc-12 is the deliberate exception to the ≤5-files rule** (29 snapshot SVGs, CI-only regen, no source change) — stated rather than silently broken.
...
| # | Flagged claim | Owner |
|---|---|---|
| **A-1** | `_DIFF_MIN_H = 29` — derived from a simulation (`styles.height = 1`), not the real S-8 implementation. ⚠️ **Residual risk is LOWER than rev-1 stated:** the Phase-2 architect lane confirmed the same floor at **W = 80, 94 and 120** (rev-1 measured two points at W=80 only), including W=94 — the exact coordinate LLR-124.4's boundary names, which neither Phase-1 lane had measured | Inc-5/Inc-10 |
| **A-2** | The fallback overlay's dismissal mechanism, which must not shadow the palette's `escape` | **Inc-5** (chosen where the code is written; **discharged** at Inc-10, where `AT-B78-25` can observe it) |
...
| **A-6** | `LLR-123.2`'s row-centring arithmetic (`_KIND_LABEL` is **no longer assumed** — re-derived at `screens_directionb.py:6637-6641`) | Inc-4 |
...
| Every **story** demonstrated RED pre-change | ✓ | §5.4's RED-today column — **9 of 9 stories**, each executed. ⚠️ **Not 31 of 31 ATs**: rev-1's checklist row said "Every AT", which Phase 2 correctly called overstated. §5.3 now reports **24 GATEs / 9 PINs**, and three rev-1 GATE labels (`AT-B78-11`, `-19`, `-22`) are relabelled PINs because they are green on `origin/main`. |
| **PINs labelled and never counted as gates** | ✓ | §5.3 — **9 PINs**: the six rev-1 PINs (`AT-B78-02, 03, 10, 12, 18, 31`), which the QA lane independently audited as **all honest, none a downgraded gate**, plus three relabelled *toward* honesty (`AT-B78-11`, `-19`, and rev-1's inert form of `-03`). **Six carry an owed mutation at a named increment gate.** No PIN was upgraded to close a gap. |
| **ID reconciliation complete, BOTH directions** | ✓ | §5.5 — 22 of 22 QA ATs (21 MERGED, 1 NEW), re-verified by the Phase-2 QA lane. §5.6 — 🆕 **the reverse audit rev-1 never ran**: 19 architect acceptance criteria (14 merged, 3 superseded-with-reason, **2 that were lost and are now restored**) plus 3 overwritten AT/TC subjects (2 restored as `AT-B78-32` / `TC-B78-47`). **A reconciliation audited in one direction is not audited.** |
...
| Normative keyword discipline | ✓ | **Executed scan, not asserted (M10).** **40** requirement `**Statement**` lines (`^- \*\*Statement` — revision 3 said 43, which counted three narrative uses of the word; the scan result is unaffected); a scan for `should\|must\|may\|might\|will\|ought\|can\|could` over all of them → **0 hits**. A scan for `shall` **outside** Statement lines → **3**, and every one is a **quotation** of requirement text elsewhere (`AT-B78-11`'s *"shall not increase"*, §5.6.1's quote of LLR-123.3, §6.4a's grep-pattern column) or §6.5's amendment text — **0 free-standing normative `shall`**. Revision 2's blanket claim was false at **10** sites; six were reworded to non-modal phrasing rather than the claim being weakened |
...
### 10.1 Blocker trajectory — three rounds, two independent lanes

| Lane | Round 1 | Round 2 | Round 3 | Final |
|---|---|---|---|---|
| **architect** | 3 blocker · 11 major · 8 minor | 1 · 3 · 4 | **0** · 1 · 3 | ✅ clean at blocker level |
| **qa-reviewer** | 1 blocker · 6 major · 8 minor | 1 · 2 · 2 | **0** · 1 · 2 | ✅ clean at blocker level |
| **security** | 0 blocker · 1 major · 1 minor | — | — | ✅ closed at revision 2 (`safe_text` made normative) |

**Acceptances weakened to reach zero: ZERO.** The QA lane's closing verdict is the one to keep:

> *The blocker count went 1 → 1 → 0 without a single acceptance being weakened to get there — every fix either moved work to where its prerequisite exists or strengthened a predicate.*

Three of the four AT arms moved by BL-2 came back **stronger** (a clipped-height co-assertion, a union of two split arms, an absorbed joint clause); `AT-B78-27` was retired **into** `AT-B78-26` with its observable preserved; and four predicates that could not fail — `AT-B78-03`, `AT-B78-19`, `AT-B78-22`, `AT-B78-28` — were rewritten rather than relabelled away. Nothing across three rounds touched scope, the story set, the regime design or the increment ordering.

### 10.2 Where the defects actually were

Every blocker in three rounds was in an **acceptance or a control**, not in a requirement and not in the measured foundation. The foundation was re-executed independently by both lanes in every round and reproduced in full each time — the 79-cell row, `C = W − rail − 6`, the 4/22 rail, chrome 5, the `W ≥ 94` floor, the 135/139 breakpoints, the content/clipped ladder, the 37-action palette. **The project's stated dominant defect class held exactly: the vacuous check, concentrated in specs and acceptances.**

### 10.3 Four control lessons, stated for reuse

These are written generally because each one recurred inside the very document that recorded it, and a lesson that only names its own instance cannot transfer.

1. **A reconciliation audited in one direction is not audited.** Merging two lanes under a "superset" premise hid two lost acceptance criteria in the lane treated as the base. The merging author is precisely the party who cannot see their own omissions, so the audit must run **both** ways, by enumeration.

2. **A register you fill in by hand measures attention, not propagation.** A propagation table whose input set comes from recollection cannot contain the site you failed to think of — and in this batch, both edits that failed to propagate were absent from the register built to prevent that.

3. **A derived register still needs a *gating* disposition, or it reports rather than gates.** Deriving the input set fixed one stage and exposed the next: the accounting was still by hand, so the table's own counts stopped reproducing against the file it shipped with. The control is complete only when it **exits non-zero** — see `enum.py` and §6.4a.

4. **A mutation that does not mutate is a vacuous counterfactual.** Verify the mutation *applied*, not merely that the test ran. Four separate mutations in this batch silently failed to apply — a subclass that could not remove an ancestor binding, an `isinstance` filter that missed a tuple-form binding, an in-place `BINDINGS` rebind after class creation, and an alignment step that is a no-op for every aligned input. Each produced a plausible reading indistinguishable from a real one until an applied-check was added.

**A fifth, specific to this stack and already encoded in §1.3 and LLR-124.1:** *a figure with no stated measurement convention is not re-derivable.* Three honest parties produced three Footer counts and two produced two breakpoints, in both cases because the coordinate space and the unit were unstated. When two careful measurements disagree, suspect the definition before the arithmetic.

### 10.4 Status

**Phase 2 closes.** The document is implementable as written: 9 HLRs, 30 LLRs, 32 live ATs (one retired), 48 TCs, 13 increments, zero open decisions, and six assumptions explicitly flagged `assumed — verify at Phase 3` with named owners. `enum.py` is committed alongside this document and is re-run, not re-read, whenever a figure or a normative term changes.
