...
| **TC-111** | US-011 AC (match) | I2 | Compare MATCH — stored 4-byte LE == computed → match, file untouched (assert input unchanged). | `tests/test_crc_check.py::test_check_reports_match_nonmutating` (NEW) |
| TC-112 | US-011 AC (mismatch) | I2 | Compare MISMATCH per output address; file untouched. | `tests/test_crc_check.py::test_check_reports_mismatch` (NEW) |
