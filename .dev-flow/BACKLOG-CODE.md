...
>
> **Last refresh: 2026-08-30 (ROUTING PASS — no batch; the code-lane findings of batch-88 and batch-89).** `origin/main` tip = **`a2a2534`** (PR [#206](https://github.com/jav201/s19_app/pull/206)). **Nothing from either batch had ever been routed into this file, and the reason is mechanical: the routing rule fires AT BATCH CLOSE.** Measured immediately before this pass, this file held **zero** occurrences of `R-88-`, `R-89-`, `batch-88` or `batch-89`. **batch-88 merged 2026-08-28** (PR [#203](https://github.com/jav201/s19_app/pull/203), `0f40624`) **but its `05-close.md` was written retroactively on 2026-08-30** — commit `8aa146f`, whose own subject says so: *"batch-88: the close-out artifacts it merged without, written after the fact and saying so"*. **batch-89 has not closed at all:** `.dev-flow/2026-08-28-batch-89/` holds `01-requirements.md`, `01-requirements-ledger.md` and `03-increments/` and **no `04-validation.md`, no `05-close.md`**, while its work is already merged (#204 `1238b7e`, #206 `a2a2534`). **Two batches of findings therefore had no close to be routed at** — this is a missed routing, not a judgement that the findings were unworthy.
>
> This pass routes the **code-lane half by hand**: **five new items** appended at the end of *Development flow OF the code — tests · CI · repo hygiene*, and **three existing bullets AMENDED in place** rather than re-minted under rival ids — the Flask viewer P3, the `case_04` `.mac` fixture P2 (its "before that branch is deleted" deadline is now falsified), and sub-item **(c)** of *Repo/worktree hygiene*. The **process-lane half** — validator rules, `V2`'s vacuity, template fields, the mutation harness, and the unclosed batch itself — is deliberately **not here**; it is owed in [`BACKLOG-PROCESS.md`](BACKLOG-PROCESS.md).
>
> ⚠ **Four figures carried into this pass did not survive re-measurement and are corrected at their items:** the suite's collection failure is scoped to the **base** anaconda env, **not** the declared gate env `s19env` (which collects 2735 tests, 0 errors); the vault's newest *evidence* asset is **2026-08-07**, not 2026-08-25 (all nineteen 2026-08-25 files are `desktop.ini`); the pruned-branch drift is **56 / 46 / 15 = 117** lines at **32–79** commits behind, not 55 / 45 / 15 at 60–114; and the `.cursor/` worktrees are **five**, four of them byte-identical copies of one diff on a 15-month-old base, not four independent stale suites. ⚠ **This pass ran no git and committed nothing.**
>
> **Earlier refresh: 2026-08-24 (batch-87 close — D-II re-author of the pilot IFC contract + IFC surface #3 `workspace_body`, CLOSED COMPLETE; second full batch under flow rev45).** `origin/main` tip at its cut = **`943e54a`** (**unchanged** — the batch rides `claude/batch-87-ifc-reauthor-surface-3`; **7 commits exist, none pushed, no PR open**, merge is an explicit post-close operator grant). **Shipped:** `D-II` **discharged** (operator-ruled re-author) — the batch-85 pilot contract re-authored **in place** under `D-87-A` with the union output **split into six** (`panel_handle`, `slots_container`, `artifact_row_set`, `unload_all_row`, `artifact_slots`, `project_row`, cardinalities re-measured at runtime); the batch-86 `screen_workspace` re-export block **forced** to follow (`LLR-87.5` — a child cannot move while its parent keeps a stale re-export under a balancing rule); **surface #3 `workspace_body` declared with 42 outputs** (11 own + 31 re-export), honest `PARENT : workspace_shell`; canon mirror `R-TUI-113` refreshed **with its not-discharged scope enumerated** + `R-TUI-114` amended + `R-TUI-115` seeded (**13 ids, one id per line**), **V22 289 → 276 of 544**, two under the ≤278 target; **UNPARSED census 2 → 1 — the `[IFC]` class is CLEARED, `[BATCHES]` survives as the negative control**. Validator `0 block · 284 notice · 15 n/a` at every gate; **0 source files** (pin empty at every gate). Gate suite (s19env, one complete 40:43 run): **2702 passed · 6 failed · 3 skipped · 21 deselected · 3 xfailed / 2714 selected**. Detail + cost n=3: `.dev-flow/2026-08-24-batch-87/05-close.md`.
>
...
## Development flow OF the code — tests · CI · repo hygiene
  ...
  - **▸ (P2, new 2026-07-30) `examples/case_04_bad_checksums/` is the ONLY example case with no `.mac` file** — 7 of the 8 cases carry one. A `firmware.mac` for it exists on the unmerged `web/flask-viewer` branch and nowhere else. Measured: `ls examples/*/ | grep -c '\.mac$'` → 1 for every case except `case_04` → 0. Either the fixture is genuinely missing (and every MAC-view pilot frame for that case is exercising an empty linkage source), or its absence is deliberate and undocumented. **Decide which before that branch is deleted** — see `.dev-flow/BRANCH-AUDIT-2026-07-30.md`.
    **⚠ AMENDED 2026-08-30 (batch-89 branch prune) — the deadline in the previous sentence no longer bites, but the decision is still owed.** That branch was **not** deleted: GitHub refused with `GH006: Protected branch`, and it is additionally archived as the tag `archive/web-flask-viewer` (commit `73edc8c`). **`examples/case_04_bad_checksums/firmware.mac` is confirmed present there and absent from `main`** — `git ls-tree -r --name-only archive/web-flask-viewer | grep case_04` lists `README.txt`, `firmware.a2l`, `firmware.mac`, `firmware.s19`, and `firmware.mac` is in the 41-path set that exists on the tag and nowhere in `main`. **So the fixture is recoverable indefinitely and the question is no longer urgent — it is merely unanswered:** is `case_04`'s missing `.mac` a genuine gap (making every MAC-view pilot frame for that case exercise an empty linkage source) or a deliberate, undocumented absence? **How to verify closed:** either `examples/case_04_bad_checksums/firmware.mac` exists on `main`, or its absence is stated as deliberate in `examples/MANIFEST.md` or the case's `README.txt`. Recovery, if wanted: `git checkout archive/web-flask-viewer -- examples/case_04_bad_checksums/firmware.mac`.
  - **▸ (P3, new 2026-07-30) an unmerged Flask web viewer exists and is referenced nowhere.** `web/flask-viewer`, last commit **2026-04-13** (predating batch-01), holds **1 187 lines**: `s19_app/web/` (app/routes/loader/session_store/a2l_utils/mac_display/cli + templates + CSS) and `tests/test_web_app.py`. No backlog item, no requirement, no vault note mentions it. **Revive as a batch, archive, or delete knowingly — but not unknowingly.** Full characterisation in `.dev-flow/BRANCH-AUDIT-2026-07-30.md`.
    **⚠ AMENDED 2026-08-30 (batch-89 branch prune) — it could NOT be deleted, and it is now archived. This is an amendment, not a second item.** The prune recorded GitHub refusing the delete with **`GH006: Protected branch`**. **Re-verified here, and consistent with that refusal: the branch still exists on the remote** — `git ls-remote --heads origin web/flask-viewer` → `73edc8c1d7ba952b3e188fa089881b0a46113547`. Its content is archived locally as the **annotated tag `archive/web-flask-viewer`**. ⚠ **`746cc55` is the TAG OBJECT, not a commit** — `git cat-file -t 746cc5581dc87295569692f42681c4b2076c0438` → `tag`; the commit it wraps is **`73edc8c`** (2026-04-13), which is the sha recorded for this branch in `.dev-flow/BRANCH-PRUNE-2026-08-30.md`. Cite `73edc8c` when you mean the code.
    **Content re-measured, and the "1 187 lines" characterisation above understates the file set.** `comm -23 <(git ls-tree -r --name-only archive/web-flask-viewer | sort) <(git ls-tree -r --name-only main | sort) | wc -l` → **41 paths that exist on the tag and nowhere in `main`**. **13 of those are the viewer proper** — `s19_app/web/{__init__,a2l_utils,app,cli,loader,mac_display,routes,session_store}.py` + `s19_app/web/static/app.css` + `s19_app/web/templates/{base,index,view}.html` + `tests/test_web_app.py`: a complete read-only Flask viewer with its own loader, session store and test suite. The other **28** are example fixtures (including `examples/case_04_bad_checksums/firmware.mac` — see the bullet above, whose deadline this amendment falsifies) plus a stray `debug-cdc3df.log`.
    **The decision this item asks for is UNCHANGED — revive, keep archived, or delete knowingly.** What changed is the cost of the third option: *delete* now additionally requires lifting branch protection on `origin`. **Nothing is at risk in the meantime** — the tag and the protected remote branch both stand, so this is no longer a race against a deletion.
    **How to verify closed:** a written ruling naming one of the three outcomes; and if the ruling is *archive*, `git tag -l 'archive/web-flask-viewer'` is non-empty **and** this bullet is marked ✅ rather than left open, because "archived" is currently true on disk while the item is still open in this file.
...
- **Repo/worktree hygiene — PARTIAL.** `direct`. State 2026-07-21: **(a)** git worktree REGISTRY cleaned for the 4 project worktrees; 3 left inert **directory shells** on disk (locked — deletable after reboot); rail-8 dir fully removed. **(b)** the PRIMARY checkout is on `ef5145b` (detached) — **restore primary→`main` (now `7954652`)** as an RC-1 closeout. **(c)** 5 stray `.cursor/worktrees/` (dyl/fkp/fmi/kxu/lyi) — **NOT pruned: `.cursor/` are the operator's IDE worktrees → need explicit operator ok.** **(d)** `backlog-consolidado-prioridad-f37dfb` NOT pruned (its PR #111 merged; other session's worktree).
  ...
  - **▸ (P3) AMENDS sub-item (c) directly above — the five `.cursor/` worktrees, characterised (routed 2026-08-30 from batch-89; measured at `2f04ebb`).** This is an amendment of (c), **not** a rival item: (c) recorded that they exist and were not pruned; this records *what is in them*, which is what an operator ruling actually needs.
    **All five are still registered and all five are detached** (`git worktree list`). ⚠ **Four of them are the SAME TREE, not four independent bodies of work.** `dyl`, `fkp`, `fmi` and `lyi` are each detached at **`851f770` (2025-05-20, "Update to README. Update to patch memory, to check and avoid memory overlaps")** and each carries an **identical** `git -C <wt> diff --numstat` of **`67  7  s19_app/core.py`**, plus the same untracked set: `.venv/`, `REQUIREMENTS.md` (2 175 bytes, mtime 2026-03-28), `requirements-dev.txt`, `s19_app.egg-info/`, `tests/`. That `tests/` is **one file** — `tests/test_core_srecord_validation.py` — not a copy of the suite. **Their base commit is 15 months old; the untracked files were written 2026-03-28/29.**
    *(⚠ The characterisation this arrived with — "four worktrees", "15-month-old copies of the test suite", "`REQUIREMENTS.md` from May 2025" — is wrong in three ways: it is **five**, the `tests/` dir is **one file** not a suite copy, and it conflates the **2025-05-20 base commit** with the **2026-03** mtimes of the untracked files. The base is 15 months old; the files are ~5 months old. Corrected here so the next reader measures rather than inherits.)*
    **`kxu` is the one that differs, and it is the one holding work worth a decision.** Detached at **`5811fd4` (2026-03-29)**, **no `.venv/` and no `.egg-info/`**, and **three MODIFIED tracked files**: `s19_app/tui.py` **`649 / 64`**, `tests/test_tui_helpers.py` **`45 / 1`**, and `REQUIREMENTS.md` **`33 / 2`** — **727 changed lines**, the third file not previously named. Plus untracked `examples/ASAP2_Demo_V161.a2l` and `examples/ASAP2_Demo_V171.a2l`. ⚠ **`s19_app/tui.py` is a MODULE there; on `main` `tui` is a PACKAGE** — the base predates that split, so this work cannot be rebased mechanically and salvage means re-siting it by hand.
    **⚠ Correction on the search-pollution claim.** These live at `C:\Users\jjgh8\.cursor\worktrees\s19_app\`, **outside the repository**, so a grep rooted at `C:\Users\jjgh8\Github\s19_app` does **not** match them — only a sweep rooted at the user home does. The repo's own `.cursor/` holds just `rules/` and a 48-byte `worktrees.json`. So the hazard is real but its trigger is a home-rooted search, not a repo-wide one; state it that way or the next reader will not reproduce it.
    **What closes it: an operator ruling, per worktree** — `dyl`/`fkp`/`fmi`/`lyi` are one diff kept four times on a 15-month-old base (drop, or keep exactly one); `kxu`'s 727 lines are salvage-by-hand or discard. **Still gated on the same consent as (c): `.cursor/` are the operator's IDE worktrees.**
    **How to verify closed:** `git worktree list` names only the primary checkout and the `.claude/worktrees/` entries, and `C:\Users\jjgh8\.cursor\worktrees\s19_app` is gone or holds only what the ruling kept. ⚠ **`git worktree remove` DISCARDS the modified and untracked files enumerated above — take the ruling before running it.**

### 🆕 Routed 2026-08-30 — code-lane findings of batch-88 and batch-89 (no batch closed; see the top-of-file refresh note)

> These five were measured during batches 88–89 and never reached this file, because the routing rule fires at batch close and neither batch closed on time. **Every figure below was re-measured at `2f04ebb` during the routing pass, and four of them corrected.** Nothing here was repaired — these are routed, not fixed.

  - **▸ (P2, routed 2026-08-30 from batch-89) `python -m pytest` cannot collect the suite in the BASE anaconda env — 22 collection errors, because `tests.conftest` resolves to a DIFFERENT REPOSITORY.**
    Executed from `C:\Users\jjgh8\Github\s19_app`: `python -m pytest --collect-only -q` → **`2495 tests collected, 22 errors`**, every error of the form
    `ImportError: cannot import name 'change_document_factory' from 'tests.conftest' (C:\Users\jjgh8\Github\mapper\tests\conftest.py)`.
    **Cause, measured not inferred:** `C:\Users\jjgh8\anaconda3\Lib\site-packages\_editable_impl_mapper.pth` contains the single line `C:\Users\jjgh8\Github\mapper`, so that directory joins `sys.path` unconditionally at interpreter start; `C:\Users\jjgh8\Github\mapper\tests\__init__.py` exists, so **`mapper`'s `tests` package wins the name** over this repo's. One-line discriminator: `python -c "import tests.conftest as c; print(c.__file__)"` → `C:\Users\jjgh8\Github\mapper\tests\conftest.py`.
    ⚠ **CORRECTION to the framing this arrived with, and it changes the priority: the DECLARED GATE ENV IS UNAFFECTED.** `C:\Users\jjgh8\anaconda3\envs\s19env\python.exe -m pytest --collect-only -q` → **`2735 tests collected in 1.04s`, 0 errors**, and the same discriminator there prints `C:\Users\jjgh8\Github\s19_app\tests\conftest.py`. `s19env`'s site-packages holds only `distutils-precedence.pth` and `__editable__.s19tool-0.1.0.pth`. **So the true statement is "the base env cannot run this repo's suite", NOT "the machine cannot run its own suite" — and no gate figure recorded in batches 86–89 is retroactively suspect, because those were all taken in `s19env`.**
    **What is wrong today even so:** `CLAUDE.md` documents the suite command as bare `pytest -q`, which is **false in the environment a fresh shell lands in**, and it fails by silently importing another project's fixtures rather than by saying the environment is wrong — the worst available failure mode, and one that costs a reader a debugging session before it names itself.
    **What closes it — an OPERATOR DECISION, because the fix reaches another project.** Either **(a)** remove the `.pth` and re-install `mapper` editable into `mapperenv` (which already exists), or **(b)** leave it and make `CLAUDE.md`'s "Common commands" name `s19env` explicitly instead of a bare `pytest`. (a) fixes the env; (b) fixes the documentation and accepts the trap. **Do not pick unilaterally — (a) changes a repository this backlog does not own.**
    **How to verify closed:** in whichever environment `CLAUDE.md` names, `python -c "import tests.conftest as c; print(c.__file__)"` prints a path under `C:\Users\jjgh8\Github\s19_app`, **and** `python -m pytest --collect-only -q` exits 0 with 0 errors. ⚠ Running that check only in `s19env` verifies nothing — it passes there today. The check must be run in the env the documentation names.

  - **▸ (P2, routed 2026-08-30 from batch-89) `[build-system] requires = ["setuptools>=61.0"]` is FALSE — a build honouring the declared floor cannot generate this project's metadata.**
    `pyproject.toml:4` declares `requires = ["setuptools>=61.0"]`. `pyproject.toml:23` declares `license = "MIT"` — the **PEP 639 SPDX-string** form. Executed against setuptools **75.1.0** (the base env), i.e. a version that satisfies the declared floor by many releases:
    `python -c "import setuptools.build_meta as m; m.prepare_metadata_for_build_wheel(tmp)"` →
    `ValueError: invalid pyproject.toml config: 'project.license'.` … `configuration error: 'project.license' must be valid exactly by one definition (2 matches found)` … `OFFENDING RULE: 'oneOf'` … `GIVEN VALUE: "MIT"`. The schema it is validated against admits only `{file = …}` or `{text = …}`.
    **Pre-existing, not a working-tree artefact:** `git show HEAD:pyproject.toml` carries the identical `requires` and `license` lines.
    **It fails BEFORE `requires-python` is ever read** — the error names `project.license` — which is why this and the item below are two defects and not one.
    **Bounded by measurement rather than asserted:** setuptools **83.0.0** (in `s19env`) passes config validation on the same file, so the real floor lies in **(75.1.0, 83.0.0]**. The documented PEP 639 support landed in setuptools **77.0.0**; that number is taken from the packaging record, **not** measured here, and the two endpoints above are what this item actually establishes.
    **Why it survives: CI cannot see it.** `pip` builds with isolation and, given only a lower bound, fetches current setuptools — so the declaration is never tested against the version it declares. The exposure is a `--no-build-isolation` build, a pinned-toolchain build, or any resolver that honours the stated minimum.
    **What closes it:** raise the declaration to `requires = ["setuptools>=77"]`, **or** revert `license` to the table form `license = {text = "MIT"}`. One line either way; the choice is whether the project wants PEP 639 metadata.
    **How to verify closed:** in a throwaway env with **exactly the declared minimum** setuptools installed, `prepare_metadata_for_build_wheel` (or `python -m build --no-isolation`) succeeds. ⚠ **A check run under current setuptools cannot fail and is not a verification** — it is the same blindness that hides the defect today.

  - **▸ (P3, routed 2026-08-30 from batch-89) the vault's pilot visual evidence is three weeks behind `main`, and the directory's newest mtime LIES about that.**
    `G:\My Drive\ConsultIA\Obsidian_Vaults\AI-Consulting-Brain\01 - Proyectos\s19_app\assets\pilot` holds **195 files**. ⚠ **Only 176 are evidence** — 160 `.svg` + 16 `.gif`. **The other 19 are `desktop.ini`**, Google-Drive metadata.
    **Newest EVIDENCE asset: 2026-08-07.** Newest file of any kind: **2026-08-25 — and all nineteen 2026-08-25 files are `desktop.ini`.** Full histogram, evidence only: `11 × 2026-07-08 · 75 × 2026-08-03 · 90 × 2026-08-07`.
    ⚠ **This is the correction that matters: a freshness check that reads the newest file in the tree reports 2026-08-25 and is wrong by 18 days**, because it is reading filesystem metadata as if it were evidence. *(The figure this item arrived with — "newest 2026-08-25" — is exactly that mistake, made in good faith.)* Against it, **batch-88 merged 2026-08-28** (PR #203, `0f40624`) and batch-89 on 2026-08-30 (#204 `1238b7e`, #206 `a2a2534`).
    **No sync will fix it on its own,** and that is the load-bearing half: `~/.claude/commands/dev-flow-sync.md:66` — **`mode: core`** → *"generate **only** the `<batch_id>-README.md` … and **skip the artifact copy entirely**."* A `core` sync therefore leaves a stale gallery and reports success.
    **What closes it:** an explicit regeneration of the pilot gallery from merged `main` through the `visual-evidence.md` §5 flattening helper — run as its own step, never as a hoped-for side effect of a `core` sync.
    **How to verify closed:** `find <pilot dir> -type f \( -name '*.svg' -o -name '*.gif' \) -printf '%TY-%Tm-%Td\n' | sort | tail -1` — **restricted to evidence extensions, never over all files** — is at or after the merge date of the newest batch that touched `s19_app/tui/`.
    **Lane note (router Amendment A):** the CAUSE-half — whether `mode: core` should *announce* that it skipped the artifact copy instead of silently leaving stale evidence behind — is a `/dev-flow-sync` command behaviour and belongs in `BACKLOG-PROCESS.md`. **Only the artifact-staleness half is written here**, per the split rule. Sibling in this file: the batch-70-sync carry about the recursive SVG copy — same surface, different defect, still open.

  - **▸ (P3 — a DECISION, not a task; routed 2026-08-30 from batch-89) `requires-python` was raised to `>=3.11` and nothing in this project can fail if it is wrong again.**
    Commit **`ad4924a`** (2026-08-29, *"s19_app: requires-python was false — the floor is raised to the version CI actually verifies"*) changed `pyproject.toml` from `requires-python = ">=3.8"` to `">=3.11"`. **Both the old value and the new are unfalsifiable by this project's CI:** `.github/workflows/tui-ci.yml:33`, `tui-ci.yml:68` and `.github/workflows/snapshot-regen.yml:31` **all pin `python-version: "3.11"`**, so `pip install -e .` evaluates the marker against 3.11 and passes. The identical setup passed the false `>=3.8` for months — that is the proof the guard is absent, not a hypothetical.
    **The mechanical floor is 3.10**, bound by `@dataclass(slots=True)` (added in Python 3.10) at **39 sites across 16 files** in `s19_app/` — `grep -rn 'slots=True' --include='*.py' s19_app/ | wc -l` → **39**; `grep -rln … | wc -l` → **16** — first at **`s19_app/compare.py:56`**. A targeted scan of `s19_app/` for 3.11-only constructs (`tomllib`, `ExceptionGroup`, `except*`, `StrEnum`, `typing.Self`, `LiteralString`, `assert_never`, `TaskGroup`) returns **zero hits**. ⚠ **That narrows the floor to 3.10; it is a scan of a named list, not a proof of absence** — stated as a bound, not a fact.
    **So `>=3.11` is not false — it is stricter than the code requires — and nothing distinguishes "correct" from "stricter than necessary" from "wrong again".**
    **Why this is routed as a DECISION and not a task:** the only construct that makes the claim testable is **a CI matrix cell at the floor whose success condition is that `pip install -e .` FAILS**. That inverts the usual green/red reading and adds a permanent cell to a ~35-minute workflow, so it is a CI-minutes-and-legibility trade that needs an operator ruling before anyone writes it. ⚠ **And it is a partial guard even then:** such a cell proves the *declaration* bites on 3.10, not that 3.11 is the *correct* floor. Proving that additionally requires the suite passing at 3.11 and failing at 3.10 on the language features themselves — a second, more expensive cell. **Rule on which of the two you are buying.**
    **How to verify closed, either way it is ruled:** either the matrix cell exists **and goes RED when `requires-python` is loosened to `>=3.10`** (a cell never shown red under its own mutation is not a guard), or the ruling is recorded in writing that the floor is asserted by declaration only and no test will ever cover it.

  - **▸ (P3, routed 2026-08-30 from batch-89) three pruned branches carried unexamined `REQUIREMENTS.md` drift — 117 lines the triage classified without reading.**
    Measured against `main:REQUIREMENTS.md` (**6 239 lines**) with `git diff --numstat main:REQUIREMENTS.md <sha>:REQUIREMENTS.md`:

    | branch (deleted 2026-08-30) | tip | lines on branch ABSENT from `main` | lines in `main` absent from branch | commits behind `main` |
    |---|---|---|---|---|
    | `claude/batch-62-report-escaping` | `ce349a6` | **56** | 1 470 | 79 |
    | `claude/batch-74-s19-app-a693ff` | `6e21dbb` | **46** | 824 | 40 |
    | `claude/at-tc-registry-guard-3a1043` | `2ccf09f` | **15** | 741 | 32 |

    ⚠ **The figures this arrived with — 55 / 45 / 15, "60–114 commits behind", "all 115" — are wrong on four of six counts.** Measured: **56 / 46 / 15**, total **117** not 115, and the behind-range is **32–79**, not 60–114. Re-derive, never carry.
    **What is wrong today:** the triage read all 117 lines as *rewording drift on stale branches* and **did not diff them line by line**. Against 1 470 / 824 / 741 lines of `main`-side movement, "reworded" is plausible — and unverified. A requirement statement that never reached `main` would look exactly like this and would be silently gone.
    **Recoverable — every tip is recorded** in `.dev-flow/BRANCH-PRUNE-2026-08-30.md`, and **all three SHAs are still reachable at this cut** (`git cat-file -e <sha>^{commit}` succeeds for each, verified during this routing pass).
    **What closes it:** read the 117 lines. Per branch, **without restoring anything**: `git diff main:REQUIREMENTS.md <sha>:REQUIREMENTS.md`. To restore instead: `git branch <name> <sha>`.
    **How to verify closed:** each of the 117 lines is dispositioned **in writing** as either *(a)* superseded by `main`, or *(b)* a requirement statement that never reached `main`. **Class (b), if non-empty, is itself a new backlog item** — closing this one by declaring the whole set to be (a) without showing the diff repeats the exact error being registered.
    ⚠ **Time-bounded.** Git's reflog holds these ~90 days from 2026-08-30; after that only the recorded SHAs plus gc survival keep them reachable. **Do this before ~2026-11-28, or accept the loss knowingly** — which is a legitimate outcome, but must be a ruling and not a lapse.
