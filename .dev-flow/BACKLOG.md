...
>
> **`origin/main` tip = `84180b4`** (batch-55 P-1b + re-freeze #104/#105 · memmap fix #106 · unload feature #107 · snapshot regen #108 · backlog reconcile #109 · **batch-57 CRC headless keel #110**). **Last refresh: 2026-07-20 (post-#110).**
>
...
### 🔺 TOP — active / parked
- **batch-58 — CRC Algorithm Designer: the Variant B TUI VIEW** (P0, next feature). `/dev-flow`. **NOT STARTED** — the headless keel (batch-57) is MERGED (#110 `84180b4`), so the view is unblocked. Scope: the Variant B TUI screen (form + live KAT verdict + coverage strip + Load/Save + load/check/save flow strip — design + prototypes already DONE in `prototypes/crc_designer.*`); **E6** legacy `crc_config` up-converter; **`emit_job`** whole-job serializer; wire the width-general kernel into the shipped `crc.py` operation; `check==compute("123456789")` enforce-on-save in the view. Pending `/dev-flow-sync` for batch-57 first.
- **Repo/worktree hygiene — ACTIVE** (parallel session, branch `claude/worktree-cleanup-next-steps-672f2c`, cut off `main` @ `84180b4`, 0 commits yet). `direct`/`/fast-dev-flow`. Open state as of 2026-07-20: **(a)** the PRIMARY checkout `…/Github/s19_app` is parked on the cleanup branch, not `main` — RC-1 wants it back on `main`; **(b)** 5 stray `.cursor/worktrees/` (dyl `851f770`, fkp `851f770`, fmi `851f770`, kxu `5811fd4`, lyi `851f770`) in detached HEAD — `git worktree remove`/`prune` candidates; **(c)** this backlog-reconcile lives in `.claude/worktrees/backlog-consolidado-prioridad-f37dfb` and must merge/close cleanly. Confirm each worktree has no unmerged work before pruning.
...
- **CRC Algorithm Designer** — the VIEW is now the TOP active item (**batch-58**, see above). The **headless keel (batch-57) is MERGED** — [#110](https://github.com/jav201/s19_app/pull/110) `84180b4`, autonomous+self-merge; shipped ADDITIVELY (0 frozen diffs, `crc.py` untouched): `crc_kernel.py` (width-general 8-64 engine + `crc_lut` fast-path E7 + 7-preset catalogue) + `crc_designer_model.py` (`CrcTemplate`/`CrcJob`/`CrcTarget`; multi-range coverage intra×join; `gap_conflict`+`evaluate_target` gap-safety E8; parse/emit/read collect-don't-abort). 45 tests; code-review 1 HIGH (F1 parse_job crash) FIXED. Design in `docs/crc-algorithm-designer/01-requirements.md` + `prototypes/crc_designer.*`.
