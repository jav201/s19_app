...
| US-WS | HLR-066 | LLR-066.7 | TC-066.7 | `s19_app/tui/app.py:6954` (primary-reload-preserving-MAC merge) · `:6997` (`_merge_mac_with_existing_primary`) | AT-066d node (below) | pass | **MJ-1** — writer-census (C-15.1) caught the carry-forward drop at Phase 2, before code |
...
| US-WS | AT-066d | MAC-merge `app.py:6954`/`:6997` → `#ws_stats` | **counterfactual (MJ-1/F-1)**: load S19 OOO=4 → attach MAC → `#ws_stats` STILL `⚠4 OOO` + entry preserved | `…::test_at066d_merge_preserves_facts` (L248) | pass |
