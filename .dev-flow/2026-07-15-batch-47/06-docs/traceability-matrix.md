...
| US-MAP | HLR-072 | LLR-072.3 | TC-072.3 | `screens_directionb.py::MapRuler:1453` | AT-072b node (below) | pass | ⚠️ **AMENDED at batch-77** (`R-TUI-072` §6.5 Amendment A): one tick per emitted run start plus one for the last mapped byte. *Before: 5 ticks @ 0/25/50/75/100 %.* C-29-measured grid = 66×14 @80×24 / 52×12 @120×30 → `0x` prefix dropped at 52 cols (C-13.1 fallback) |
...
| US-MAP | AT-072b | `MapRuler` widget | ⚠️ **RE-DERIVED at batch-77**: labels ⊆ emitted run starts ∪ last mapped byte, strictly ascending, distinct, none narrower than its label. *Before: ruler renders **exactly 5** ticks; first == span start, last == span end.* | `…::test_at072b_ruler` (L299) | pass |
