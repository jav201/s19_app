...
| **Address ruler** | A NEW widget beneath the strip: one tick label per emitted **run** start plus one for the **last mapped byte** (`span_end - 1`, never the exclusive `span_end`); no tick renders narrower than its 8-hex-digit label. ⚠️ **AMENDED at batch-77** (`R-TUI-072` §6.5 Amendment A). *Before, as batch-47 shipped it: exactly 5 tick labels at 0/25/50/75/100 % of the address span; tick 0 % == span start, tick 100 % == span end — under which **4 of the 5** labels named addresses in no mapped range, and the 100 % tick named the exclusive `span_end`.* | `screens_directionb.py::MapRuler:1453` |
