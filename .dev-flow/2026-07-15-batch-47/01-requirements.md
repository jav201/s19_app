...
- **Before → After (exact hex per class; rules at `styles.tcss:519-536` post-theme — `.sev-ok` :519, `.sev-error` :523, `.sev-warning` :527, `.sev-info` :531, `.sev-neutral` :535, `.mac_out_of_range` :542. The `:499-515` cite in LLR-065.4 §4 is the PRE-theme location, retained there as the Before-state reference):**
...
- **Preserved unchanged:** `.mac_out_of_range` stays `#d9a35b`; `band-*` rules untouched (separate entropy domain). All `sev-*` class NAMES unchanged; every hue stays in its correct colour family.
- **⚠ CORRECTION (2026-07-16, final PR-QA / Inc-10 probe) — this block originally claimed "the `Orange = MAC warning` cue survives" via `.mac_out_of_range`. That claim was FALSE.** `.mac_out_of_range` paints ONLY the Sections-list "MAC out-of-range @ 0x…" labels (`app.py:8553`); the MAC/Issues **warning ROWS** are painted `css_class_for_severity(WARNING)` = `.sev-warning`, which this amendment rebound to pale yellow. So the restyle DID change the documented row cue — resolved by **Amendment F** (operator decision: keep yellow, amend the convention), not by this block. Both the Inc-8 code-review and the orchestrator asserted the survival without checking what `.mac_out_of_range` actually paints.
...
**Amendment F — Warning severity renders PALE YELLOW; "Orange" is re-scoped to the MAC-specific cues (US-FND / HLR-065, LLR-065.4 + the REQUIREMENTS severity convention — operator decision 2026-07-16, applied Inc-10)**
- **Before (REQUIREMENTS.md severity conventions):** *"MAC row colouring adds **Orange** for warning-level overlap/alias/symbol-only-in-MAC findings."* — i.e. the warning-level **row** cue was documented as Orange (`.sev-warning` = `#d9a35b`).
- **After:** warning-level rows (MAC **and** Issues) render the approved pastel **YELLOW `#f6ff8f`** via `.sev-warning` — consistent with the operator-approved navy/pastel palette. **ORANGE is re-scoped, not deleted:** it remains the **MAC-specific** cue on (a) the MAC record `⚠` status glyph (`orange3`, Inc-5), (b) the hex-view MAC address overlay (`MAC_ADDRESS_OVERLAY_STYLE` = `orange3`, **frozen** `color_policy.py`), and (c) the Sections-list "MAC out-of-range @ 0x…" labels (`.mac_out_of_range` `#d9a35b`, unchanged).
- **Why this amendment exists:** Inc-8's palette rebound `.sev-warning` orange→yellow. Amendment C wrongly asserted the row cue survived via `.mac_out_of_range` — it does not paint those rows. Rather than ship a documented-convention violation (or a false amendment), the convention is amended to describe what actually renders. Surfaced by the Inc-10 live-app probe (`WARNING → .sev-warning → Color(246,255,143)`), NOT averaged away.
- **Deleted / New tokens:** Deleted — "Orange = warning-level MAC **row**". New — "Yellow (`#f6ff8f`) = warning severity (rows, all screens)"; "Orange (`#d9a35b`/`orange3`) = MAC-specific cues (⚠ glyph · hex overlay · out-of-range section labels)".
- **Parent-HLR re-read:** HLR-065 / LLR-065.4 mandate `sev-*` class NAMES + severity SEMANTICS preserved — **satisfied**: names unchanged, `color_policy.py` frozen 0-diff, the severity MEANING (warning) is unchanged; only its rendered hue moves, which is precisely what §6.5 exists to record. REQUIREMENTS.md's severity-convention section is updated to match.
- **Re-derived reqs:** LLR-065.4 · **NEW AT-065c** (binds every legend colour LABEL to the hue its severity class actually resolves to, by HSV family — closes the gap that let the stale label through: AT-065b probed only `sev-error`) · the legend label `"Orange"` → `"Pale yellow"` (`legend.py`; `"Yellow"` was unusable — it is the Hex focus-highlight row's key and would collide with the `COLOUR_SEVERITY` invariant TC-322 asserts).
