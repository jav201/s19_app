...
| Snapshot drift | 29 batch-47 theme-drift tc016s cells `xfail(strict=False)`, 0 xpassed (non-masking) → 1 canonical-CI regen follow-up PR |
...
- **Inc-3 / Inc-6 F-nits (LOW).** Dead-code cleanup (removed unused `build_coverage_bar_text`/`coverage_bar_cells` + its test at Inc-3) and a docstring over-claim correction (hex-peek "16-align" claim vs behavior at Inc-6). Behavior unchanged; own-mess-only.
