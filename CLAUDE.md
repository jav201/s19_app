...
- **Engine-frozen guard.** These parsing-layer engine modules are git-frozen against modification by the `_ENGINE_PATHS` guards (`tests/test_engine_unchanged.py`, `tests/test_tui_directionb.py::test_tc031_*`): the frozen set is `core.py`, `hexfile.py`, `range_index.py`, `validation/`, `tui/a2l.py`, `tui/mac.py`, and `tui/color_policy.py`. Any TUI-side batch that diffs one of these vs `main` trips the guards, so new code that would edit them must live elsewhere — e.g. the Intel HEX emission logic (batch-10) went in `tui/changes/io.py`, next to `emit_s19_from_mem_map`, not in `hexfile.py` (which stays the reader/oracle only). **`tui/a2l.py` was temporarily UNFROZEN** for the a2l-missing-length-fix / a2l-record-layout-length batches (operator-approved parsing-logic fixes) and **RE-FROZEN in batch-50 (P-2)** once its sanctioned edits (incl. the F841 dead-store cleanup) landed; it is a read-only oracle again. Keep it the canonical A2L module (add public symbols here + re-export from the facades); any further edit now needs an explicit unfreeze.
