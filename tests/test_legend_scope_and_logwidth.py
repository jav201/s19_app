...
screen paints — A2L→A2L, MAC→MAC, Issues→Issues, Map→Hex — and falls back to the
full table on an unmapped screen (Workspace). Row colours still round-trip
...
def test_n1_legend_scoped_per_screen(tmp_path: Path) -> None:
    ...
    assert headings["map"] == ["Hex"], headings["map"]
