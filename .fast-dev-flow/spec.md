...
- [ ] **AC-4 (US-B, containment):** When the regenerated baselines are diffed against the committed set, **only** the genuinely-changed screens (patch @80×24 + @120×30, MAC @82-col) shall move; if any unrelated screen moves, that is the wrong-env signal and the batch STOPS (does not commit).
...
| Notes | Route=/fast-dev-flow, non-blocking job, workflow_dispatch artifact (operator AskUserQuestion 2026-07-03). RC-1 PASS @ 0c06b48. Regen is CI-env-only. Prior closed spec archived → archive/2026-06-25-spec.md. **Inc 1 APPROVED 2026-07-03**: tui-ci.yml (perms + non-blocking snapshot job) + snapshot-regen.yml (new); security-reviewer OK-to-ship 0 blockers; local smoke 28-skip exit-0. Defaults adopted: update R-TUI-032 (not R-TUI-029); US-D vault → sync-time. **BLOCKED on CI round-trip** (workflow_dispatch needs the regen workflow on the DEFAULT branch → merge Inc 1 first). |
