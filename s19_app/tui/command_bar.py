...
batch-02-direction-b-restyle, increment 4:
  ...
  - ``CommandBar`` — now the PALETTE HOST and nothing else: a type-to-filter
    command palette (``Ctrl+K``, LLR-003.2 / LLR-003.3). batch-79 Inc-10
    (``HLR-118``) deleted ``#command_bar_row`` and with it the context labels,
    the find input (``/``) and the go-to input (``g``). The labels live on
    ``#status_context`` and the Loaded panel's project row (``HLR-120``); the
...
class CommandBar(Vertical):
    ...
    Summary:
        ...
        ``ListView``), mounted above the rail and the workspace body and hidden
        until ``Ctrl+K`` opens it (LLR-003.1). The widget is presentational — it
        emits messages and never touches the engine.

        batch-79 Inc-10 (``HLR-118``) deleted the bar ROW this widget used to
        carry — the ``"›"`` prompt, the context labels, the find and go-to
        inputs — because each duplicated a per-screen surface, and the find in
        particular wrote into the workspace pane whatever screen was active.
        The widget itself and ``#command_bar_slot`` survive: the palette is the
        one surface with no per-screen equivalent.
    ...
    Data Flow:
        - ``compose`` yields the hidden palette dropdown (trigger input +
    ...
    def compose(self) -> ComposeResult:
        # batch-79 Inc-10 (HLR-118): `#command_bar_row` is DELETED. It carried a
        # prompt glyph, the two context labels and a find/go-to input pair that
        # duplicated what each screen already owns — and worse than duplicated:
        # the bar's find wrote into the WORKSPACE pane regardless of which
        # screen was active (fixed at Inc-9, HLR-119).
        #
        # Nothing here was deleted before its replacement existed AND was
        # observed: the context labels moved to `#status_context` and the Loaded
        # panel's project row at Inc-7, the keys re-homed onto each screen's own
        # inputs at Inc-9, and the 14 test readers re-pointed at Inc-8 while all
        # three surfaces were live.
        #
        # `#command_palette` SURVIVES, and so do this widget and
        # `#command_bar_slot` — they exist to host it. That is why the CSS
        # deletion is `:66-103` and not `:55-102`: the two container rules sit
        # ABOVE the span and stay.
